<?php
 
 /**
 * INFORMASI
 *
 * Nama Aplikasi :  HRIS
 * Email Support :  hris@karyasoftware.com
 * Developer     :  Nizar Basyrewan, S.Si - 0895 606460 731
 * Tahun         :  2020
 * Copyright     :  ©karyasoftware.com. All Rights Reserved
 */

if ( ! defined('BASEPATH')) exit('No direct script access allowed');

class Assets extends MY_Controller
{

   /*Function to set JSON output*/
	public function output($Return=array()){
		/*Set response header*/
		header("Access-Control-Allow-Origin: *");
		header("Content-Type: application/json; charset=UTF-8");
		/*Final JSON response*/
		exit(json_encode($Return));
	}
	
	public function __construct()
     {
          parent::__construct();
          //load the models
          $this->load->model('Core_model');
		  $this->load->model('Employees_model');
		  $this->load->model('Department_model');
		  $this->load->model('Assets_model');
		  $this->load->model('Custom_fields_model');
     }

    // =====================================================================================================	
    // ASET
    // =====================================================================================================
	 	
		public function index() 
		{
			$session = $this->session->userdata('username');
			if(empty($session)){
				redirect('admin/');
			}
			$system = $this->Core_model->read_setting_info(1);
			if($system[0]->module_assets!='true'){
				redirect('admin/dashboard');
			}
			$data['title']         = $this->lang->line('xin_assets').' | '.$this->Core_model->site_title();
			$data['icon']          = '<i class="fa fa-cubes"></i>';
			$data['desc']          = 'DATA : Asset';
			$data['breadcrumbs']   = $this->lang->line('xin_assets');
			$data['path_url']      = 'assets';
			
			$role_resources_ids    = $this->Core_model->user_role_resource();

			$data['all_assets_categories'] = $this->Assets_model->get_all_assets_categories();
			
			if(in_array('25',$role_resources_ids)) {
				if(!empty($session)){ 
					$data['subview'] = $this->load->view("admin/assets/assets_list", $data, TRUE);
					$this->load->view('admin/layout/layout_main', $data); //page load
				} else {
					redirect('admin/');
				}
			} else {
				redirect('admin/dashboard');
			}
		}

		// assets list
		public function assets_list()
	    {

			$session = $this->session->userdata('username');
			if(empty($session)){ 
				redirect('admin/');
			}
			$data['title'] = $this->Core_model->site_title();
			if(!empty($session)){ 
				$this->load->view("admin/languages/languages_list", $data);
			} else {
				redirect('admin/');
			}
			// Datatables Variables
			$draw = intval($this->input->get("draw"));
			$start = intval($this->input->get("start"));
			$length = intval($this->input->get("length"));
			
			$role_resources_ids = $this->Core_model->user_role_resource();
			$user_info = $this->Core_model->read_user_info($session['user_id']);
			if($user_info[0]->user_role_id==1){
				$assets = $this->Assets_model->get_assets();
			} else {
				if(in_array('265',$role_resources_ids)) {
					$assets = $this->Assets_model->get_company_assets($user_info[0]->company_id);
				} else {
					$assets = $this->Assets_model->get_employee_assets($session['user_id']);
				}
			}
			$data = array();
			
	          foreach($assets->result() as $r) {						
				
				// get category
				$assets_category = $this->Assets_model->read_assets_category_info($r->assets_category_id);
				if(!is_null($assets_category)){
					$category = $assets_category[0]->category_name;
				} else {
				 	$category = '--';	
				}
				
				if($r->is_working==1){
					$working = '<span class="badge bg-green"> <i class="fa fa-check"></i> Bagus</span>';
				} else if($r->is_working==2){
					$working = '<span class="badge bg-red"> <i class="fa fa-check"></i> Rusak</span>';
				} else if($r->is_working==3){
					$working = '<span class="badge bg-yellow"> <i class="fa fa-check"></i> Dalam perbaikan</span>';
				} else if($r->is_working==4){
					$working = '<span class="badge bg-dark"> <i class="fa fa-check"></i> Hilang</span>';
				} 

				if($r->is_pinjam==1){
					$pinjam = '<span class="badge bg-yellow"> <i class="fa fa-minus-cicle"></i> Dipinjam</span>';
				} else if($r->is_pinjam==0){
					$pinjam = '<span class="badge bg-green"> <i class="fa fa-check"></i> Tersedia</span>';
				} 
								
				if(in_array('263',$role_resources_ids)) { //edit
					$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_edit').'">
								<button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".edit-modal-data" data-field_id="'. $r->assets_id . '">
									<span class="fa fa-pencil"></span> Edit
								</button>
							</span>';
				} else {
					$edit = '';
				}

				if(in_array('264',$role_resources_ids)) { // delete
					$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'">
									<button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->assets_id . '">
										<span class="fa fa-trash"></span>
									</button>
								</span>';
				} else {
					$delete = '';
				}

				if(in_array('265',$role_resources_ids)) { //view
					$view = '  <span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view').'">
									<button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-asset_id="'. $r->assets_id . '">
											<span class="fa fa-eye"></span>
									</button>
								</span>';
				} else {
					$view = '';
				}

				$combhr = $edit.$view.$delete;
				$created_at = $this->Core_model->set_date_format($r->created_at);
				$iname = $r->name.'<br>'.$r->manufacturer;					 			  				
				
				if($r->asset_image!='' && $r->asset_image!='no file') {
						$ol = '<a href="javascript:void(0);">
									<span class="avatar box-16">
										<img src="'.base_url().'uploads/asset_image/'.$r->asset_image.'" class="user-image-hr52" alt="">
									</span>
								</a>';
				} else {
					
					$ol = '<a href="javascript:void(0);">
								<span class="avatar box-16">
									<img src="'.base_url().'uploads/asset_image/no_image.png" class="user-image-hr52" alt="">
								</span>
							</a>';
				}
								
				$aset_foto = $ol;

				$data[] = array(
					$combhr,
					$aset_foto,
					$r->company_asset_code,
					$iname,
					$category,
					date("d-m-Y",strtotime($r->purchase_date)),					
					$working,
					$pinjam
					
				);
			}
	          $output = array(
	               "draw" => $draw,
	                 "recordsTotal" => $assets->num_rows(),
	                 "recordsFiltered" => $assets->num_rows(),
	                 "data" => $data
	            );
	          echo json_encode($output);
	          exit();
	    }

	    public function asset_read()
		{
			$session = $this->session->userdata('username');
			if(empty($session)){ 
				redirect('admin/');
			}
			$data['title'] = $this->Core_model->site_title();
			$id = $this->input->get('asset_id');
			$result = $this->Assets_model->read_assets_info($id);
			$data = array(
					'assets_id' => $result[0]->assets_id,
					'assets_category_id' => $result[0]->assets_category_id,					
					'company_asset_code' => $result[0]->company_asset_code,
					'name' => $result[0]->name,
					'purchase_date' => $result[0]->purchase_date,
					'invoice_number' => $result[0]->invoice_number,
					'manufacturer' => $result[0]->manufacturer,
					'serial_number' => $result[0]->serial_number,
					'warranty_end_date' => $result[0]->warranty_end_date,
					'asset_note' => $result[0]->asset_note,
					'asset_image' => $result[0]->asset_image,
					'is_working' => $result[0]->is_working,
					'created_at' => $result[0]->created_at,
					'all_employees' => $this->Core_model->all_employees(),
					'all_assets_categories' => $this->Assets_model->get_all_assets_categories(),
					'all_companies' => $this->Core_model->get_companies()
					);
			if(!empty($session)){ 
				$this->load->view('admin/assets/dialog_asset', $data);
			} else {
				redirect('admin/');
			}
		}		
		
		public function add_asset() 
		{
			if($this->input->post('add_type')=='add_asset') {	

				/* Define return | here result is used to return user data and error for error message */
				$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
				$Return['csrf_hash'] = $this->security->get_csrf_hash();
					
				/* Server side PHP input validation */		
				if($this->input->post('category_id')==='') {
		        	$Return['error'] = $this->lang->line('xin_error_category_field');
				
				} else if($this->input->post('asset_name')==='') {
		        	$Return['error'] = $this->lang->line('xin_error_asset_name_field');
				
				}

				if($Return['error']!=''){
		       		$this->output($Return);
		    	}
				if(is_uploaded_file($_FILES['asset_image']['tmp_name'])) {
					//checking image type
					$allowed =  array('png','jpg','jpeg','gif');
					$filename = $_FILES['asset_image']['name'];
					$ext = pathinfo($filename, PATHINFO_EXTENSION);
					
					if(in_array($ext,$allowed)){
						$tmp_name = $_FILES["asset_image"]["tmp_name"];
						$asset_image = "uploads/asset_image/";
						// basename() may prevent filesystem traversal attacks;
						// further validation/sanitation of the filename may be appropriate
						$lname = basename($_FILES["asset_image"]["name"]);
						$newfilename = 'asset_image_'.round(microtime(true)).'.'.$ext;
						move_uploaded_file($tmp_name, $asset_image.$newfilename);
						$fname = $newfilename;
					} else {
						$Return['error'] = $this->lang->line('xin_error_asset_image_attachment');
					}
				} else {
					$fname = '';
				}
			
				// set data
				$data = array(
					'assets_category_id' => $this->input->post('category_id'),
					'name' => $this->input->post('asset_name'),
					'company_asset_code' => $this->input->post('company_asset_code'),
					'is_working' => $this->input->post('is_working'),			
					'purchase_date' => $this->input->post('purchase_date'),
					'invoice_number' => $this->input->post('invoice_number'),
					'manufacturer' => $this->input->post('manufacturer'),
					'serial_number' => $this->input->post('serial_number'),
					'warranty_end_date' => $this->input->post('warranty_end_date'),
					'asset_note' => $this->input->post('asset_note'),
					'asset_image' => $fname,
					'created_at' => date('d-m-Y h:i:s')
				);
				
				$iresult = $this->Assets_model->add_asset($data);
				if ($iresult) {
					$Return['result'] = 'Asset Baru Behasil Ditambahkan';
					
				} else {
					$Return['error'] = $this->lang->line('xin_error_msg');
				}

				$this->output($Return);
				exit;
			}
		}
		
		public function update_asset() 
		{
			if($this->input->post('edit_type')=='update_asset') {		
			/* Define return | here result is used to return user data and error for error message */
			$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');		
			$id = $this->uri->segment(4);
			$Return['csrf_hash'] = $this->security->get_csrf_hash();
				
			/* Server side PHP input validation */		
			if($this->input->post('category_id')==='') {
	        	$Return['error'] = $this->lang->line('xin_error_category_field');
			} else if($this->input->post('asset_name')==='') {
	        	$Return['error'] = $this->lang->line('xin_error_asset_name_field');			
			
			} else {

				
				if(is_uploaded_file($_FILES['asset_image']['tmp_name'])) {
					//checking image type
					$allowed =  array('png','jpg','jpeg','gif');
					$filename = $_FILES['asset_image']['name'];
					$ext = pathinfo($filename, PATHINFO_EXTENSION);
					
					if(in_array($ext,$allowed)){

						$tmp_name = $_FILES["asset_image"]["tmp_name"];
						$asset_image = "uploads/asset_image/";
						// basename() may prevent filesystem traversal attacks;
						// further validation/sanitation of the filename may be appropriate
						$lname = basename($_FILES["asset_image"]["name"]);
						$newfilename = 'asset_image_'.round(microtime(true)).'.'.$ext;
						move_uploaded_file($tmp_name, $asset_image.$newfilename);
						$fname = $newfilename;
						
						// set data
						$data = array(
							'assets_category_id' => $this->input->post('category_id'),
							'name' => $this->input->post('asset_name'),
							'company_asset_code' => $this->input->post('company_asset_code'),
							'is_working' => $this->input->post('is_working'),
							
							'purchase_date' => $this->input->post('purchase_date'),
							'invoice_number' => $this->input->post('invoice_number'),
							'manufacturer' => $this->input->post('manufacturer'),
							'serial_number' => $this->input->post('serial_number'),
							'warranty_end_date' => $this->input->post('warranty_end_date'),
							'asset_note' => $this->input->post('asset_note'),
							'asset_image' => $fname
						);
						
						$result = $this->Assets_model->update_assets_record($data,$id);

						
					} else {
						$Return['error'] = $this->lang->line('xin_error_asset_image_attachment');
					}
				}
			}
							
			if($Return['error']!=''){
	       		$this->output($Return);
	    	}
			
			if ($result == TRUE) {
				$Return['result'] = $this->lang->line('xin_success_asset_added');
			} else {
				$Return['error'] = $this->lang->line('xin_error_msg');
			}
			$this->output($Return);
			exit;
			}
		}
		
		public function delete_asset() 
		{
			if($this->input->post('type')=='delete_record') {
				/* Define return | here result is used to return user data and error for error message */
				$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
				$id = $this->uri->segment(4);
				$Return['csrf_hash'] = $this->security->get_csrf_hash();
				$result = $this->Assets_model->delete_assets_record($id);
				if(isset($id)) {
					$Return['result'] = $this->lang->line('xin_success_asset_deleted');
				} else {
					$Return['error'] = $this->lang->line('xin_error_msg');
				}
				$this->output($Return);
			}
		}

	// =====================================================================================================	
    // KATEGORI ASET
    // ===================================================================================================== 
	
		public function category() 
		{
			$session = $this->session->userdata('username');
			if(empty($session)){
				redirect('admin/');
			}
			$data['title']         = 'Kategori Aset | '.$this->Core_model->site_title();
			$data['desc']   	   = 'FORM INPUT : Kategori Aset';
			$data['breadcrumbs']   = 'Kategori Aset';
			$data['icon']          = '<i class="fa fa-cube"></i>';
			$data['path_url']      = 'assets_category';

			$data['all_companies'] = $this->Core_model->get_companies();
			
			$role_resources_ids    = $this->Core_model->user_role_resource();
			if(in_array('26',$role_resources_ids)) {
				if(!empty($session)){ 
					$data['subview'] = $this->load->view("admin/assets/assets_category_list", $data, TRUE);
					$this->load->view('admin/layout/layout_main', $data); //page load
				} else {
					redirect('admin/');
				}
			} else {
				redirect('admin/dashboard');
			}
		}
		
		public function category_list()
	    {

			$session = $this->session->userdata('username');
			if(empty($session)){ 
				redirect('admin/');
			}
			$data['title'] = $this->Core_model->site_title();
			if(!empty($session)){ 
				$this->load->view("admin/assets/assets_category_list", $data);
			} else {
				redirect('admin/');
			}
			// Datatables Variables
			$draw = intval($this->input->get("draw"));
			$start = intval($this->input->get("start"));
			$length = intval($this->input->get("length"));
			
			
			$assets_category = $this->Assets_model->get_assets_categories();
			$role_resources_ids = $this->Core_model->user_role_resource();
			$data = array();
			
	          foreach($assets_category->result() as $r) {						
			  			
				if(in_array('267',$role_resources_ids)) { //edit
					$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_edit').'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".payroll_template_modal" data-field_id="'. $r->assets_category_id . '"><span class="fa fa-pencil"></span></button></span>';
				} else {
					$edit = '';
				}
				if(in_array('268',$role_resources_ids)) { // delete
					$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->assets_category_id . '"><span class="fa fa-trash"></span></button></span>';
				} else {
					$delete = '';
				}
				
				$combhr = $edit.$delete;
										 			  				
				$data[] = array($combhr,
					$r->category_name
				);
			}
	          $output = array(
	               "draw" => $draw,
	                 "recordsTotal" => $assets_category->num_rows(),
	                 "recordsFiltered" => $assets_category->num_rows(),
	                 "data" => $data
	            );
	          echo json_encode($output);
	          exit();
	    }
	   
		public function add_category() 
		{
			if($this->input->post('add_type')=='add_category') {		
			/* Define return | here result is used to return user data and error for error message */
			$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
			$Return['csrf_hash'] = $this->security->get_csrf_hash();
				
			/* Server side PHP input validation */		
			if($this->input->post('name')==='') {
	        	$Return['error'] = $this->lang->line('xin_error_cat_name_field');
			}
							
			if($Return['error']!=''){
	       		$this->output($Return);
	    	}
			
			// set data
			$data = array(
			'category_name' => $this->input->post('name'),
			'created_at' => date('d-m-Y h:i:s')
			);
			
			$result = $this->Assets_model->add_assets_category($data);
			if ($result == TRUE) {
				$Return['result'] = $this->lang->line('xin_success_assets_category_added');
			} else {
				$Return['error'] = $this->lang->line('xin_error_msg');
			}
			$this->output($Return);
			exit;
			}
		}
				
		public function update_assets_category() 
		{
			if($this->input->post('edit_type')=='assets_category') {		
				/* Define return | here result is used to return user data and error for error message */
				$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
					
				$id = $this->uri->segment(4);
				$Return['csrf_hash'] = $this->security->get_csrf_hash();
					
				/* Server side PHP input validation */		
				if($this->input->post('name')==='') {
		        	$Return['error'] = $this->lang->line('xin_error_cat_name_field');
				}
								
				if($Return['error']!=''){
		       		$this->output($Return);
		    	}
				
				// set data
				$data = array(
					'category_name' => $this->input->post('name')
				);
				
				$result = $this->Assets_model->update_assets_category_record($data,$id);
				if ($result == TRUE) {
					$Return['result'] = $this->lang->line('xin_success_assets_category_updated');
				} else {
					$Return['error'] = $this->lang->line('xin_error_msg');
				}
				$this->output($Return);
				exit;
			}
		}
	
		public function read_assets_category() 
		{
			$session = $this->session->userdata('username');
			if(empty($session)){ 
				redirect('admin/');
			}
			$data['title'] = $this->Core_model->site_title();
			$id = $this->input->get('assets_category_id');
			$result = $this->Assets_model->read_assets_category_info($id);
			$data = array(
					'assets_category_id' => $result[0]->assets_category_id,
					'company_id' => $result[0]->company_id,
					'category_name' => $result[0]->category_name,
					'created_at' => $result[0]->created_at
					);
			if(!empty($session)){ 
				$this->load->view('admin/assets/dialog_assets_category', $data);
			} else {
				redirect('admin/');
			}
		}

		public function delete_assets_category() 
		{
			if($this->input->post('type')=='delete_record') {
				/* Define return | here result is used to return user data and error for error message */
				$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
				$id = $this->uri->segment(4);
				$Return['csrf_hash'] = $this->security->get_csrf_hash();
				$result = $this->Assets_model->delete_assets_category_record($id);
				if(isset($id)) {
					$Return['result'] = $this->lang->line('xin_success_assets_category_deleted');
				} else {
					$Return['error'] = $this->lang->line('xin_error_msg');
				}
				$this->output($Return);
			}
		}

	// =====================================================================================================	
    // PINJAM ASET
    // ===================================================================================================== 
	
		public function pinjam() 
		{
			$session = $this->session->userdata('username');
			if(empty($session)){
				redirect('admin/');
			}

			$data['title']         = 'Pinjam | '.$this->Core_model->site_title();
			$data['desc']   	   = 'FORM INPUT : Proses Pinjam Aset';
			$data['breadcrumbs']   = 'Pinjam ';
			$data['icon']          = '<i class="fa fa-mail-forward"></i>';			
			$data['path_url']      = 'assets_pinjam';

			$data['all_companies']         = $this->Core_model->get_companies();
			$data['all_employees']         = $this->Core_model->all_employees();

			$data['all_assets_categories'] = $this->Assets_model->get_all_assets_categories();
			$data['all_assets']            = $this->Assets_model->get_all_assets();

			$role_resources_ids    = $this->Core_model->user_role_resource();
			if(in_array('27',$role_resources_ids)) {
				if(!empty($session)){ 
					$data['subview'] = $this->load->view("admin/assets/assets_pinjam_list", $data, TRUE);
					$this->load->view('admin/layout/layout_main', $data); //page load
				} else {
					redirect('admin/');
				}
			} else {
				redirect('admin/dashboard');
			}
		}
		
		public function pinjam_list()
	    {

			$session = $this->session->userdata('username');
			if(empty($session)){ 
				redirect('admin/');
			}
			$data['title'] = $this->Core_model->site_title();
			if(!empty($session)){ 
				$this->load->view("admin/assets/assets_pinjam_list", $data);
			} else {
				redirect('admin/');
			}
			// Datatables Variables
			$draw = intval($this->input->get("draw"));
			$start = intval($this->input->get("start"));
			$length = intval($this->input->get("length"));
			
			
			$assets_pinjam = $this->Assets_model->get_assets_pinjam();
			$role_resources_ids = $this->Core_model->user_role_resource();
			$data = array();
			
	          foreach($assets_pinjam->result() as $r) {	

	          	// get company
				$company = $this->Core_model->read_company_info($r->company_id);
				if(!is_null($company)){
					$comp_name = $company[0]->name;
				} else {
				 	$comp_name = '--';	
				}

				$user = $this->Core_model->read_user_info($r->employee_id);				
				if(!is_null($user)){
					$nip       = $user[0]->employee_id;
					$full_name = $user[0]->first_name.' '.$user[0]->last_name;
				} else {
					$nip       = '--';
					$full_name = '--';	
				}

				$asset_pinjam = $this->Core_model->read_asset_info($r->assets_id);				
				if(!is_null($asset_pinjam)){
					$asset_pinjam_name = $asset_pinjam[0]->name;
					$asset_pinjam_kode = $asset_pinjam[0]->company_asset_code;
				} else {
					$asset_pinjam_name = '--';
					$asset_pinjam_kode = '--';	
				}					
			  			
				

				if(in_array('277',$role_resources_ids)) { //edit
					$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_edit').'">
								<button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".edit-modal-data" data-field_id="'. $r->pinjam_id . '">
									<span class="fa fa-pencil"></span> Edit
								</button>
							</span>';
				} else {
					$edit = '';
				}

				if(in_array('278',$role_resources_ids)) { // delete
					$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'">
									<button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->pinjam_id . '">
										<span class="fa fa-trash"></span>
									</button>
								</span>';
				} else {
					$delete = '';
				}

				if(in_array('279',$role_resources_ids)) { //view
					$view = '  <span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view').'">
									<button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-pinjam_id="'. $r->pinjam_id . '">
											<span class="fa fa-eye"></span>
									</button>
								</span>';
				} else {
					$view = '';
				}

				$combhr = $edit.$view.$delete;

				if ($r->kembali_date == '0000-00-00'){
					$kembali = 'Belum Dikembalikan';
				} else {
					$kembali =  date("d-m-Y",strtotime($r->kembali_date));
				}

				if ($r->is_pinjam == '1'){
					$status = 'Dipinjam';
				} else {
					$status = 'Dikembalikan';
				}
										 			  				
				$data[] = array(
					$combhr,
					date("d-m-Y",strtotime($r->pinjam_date)),
					$kembali,
					$nip,
					$full_name,
					$asset_pinjam_kode,
					$asset_pinjam_name,
					$status
				);
			}
	          $output = array(
	               "draw" => $draw,
	                 "recordsTotal" => $assets_pinjam->num_rows(),
	                 "recordsFiltered" => $assets_pinjam->num_rows(),
	                 "data" => $data
	            );
	          echo json_encode($output);
	          exit();
	    }

	    public function asset_pinjam_read()
		{
			$session = $this->session->userdata('username');
			if(empty($session)){ 
				redirect('admin/');
			}
			$data['title'] = $this->Core_model->site_title();
			$id            = $this->input->get('pinjam_id');
			$result        = $this->Assets_model->read_assets_pinjam_info($id);
			$data = array(

					'pinjam_id'     => $result[0]->pinjam_id,

					'company_id'     => $result[0]->company_id,
				    'employee_id'    => $result[0]->employee_id,
					
					'category_id'   => $result[0]->category_id,
					'assets_id'     => $result[0]->assets_id,
					
					'pinjam_date'   => $result[0]->pinjam_date,
					'kembali_date'  => $result[0]->kembali_date,

					'is_pinjam'     => $result[0]->is_pinjam,

					'asset_note'    => $result[0]->asset_note,
					
					'created_at'    => $result[0]->created_at,
					
					'all_companies'  => $this->Core_model->get_companies(),
					'all_employees'  => $this->Core_model->all_employees(),
					'all_categories' => $this->Assets_model->get_all_assets_categories(),
					'all_assets'     => $this->Assets_model->get_all_assets()
					
			);
			if(!empty($session)){ 
				$this->load->view('admin/assets/dialog_asset_pinjam', $data);
			} else {
				redirect('admin/');
			}
		}
	   
		public function add_asset_pinjam() 
		{
			if($this->input->post('add_type')=='add_asset_pinjam') {		
				/* Define return | here result is used to return user data and error for error message */
				$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
				$Return['csrf_hash'] = $this->security->get_csrf_hash();
					
				/* Server side PHP input validation */		
				if($this->input->post('company_id')==='') {
		        	$Return['error'] = 'Nama Perusahaan Wajib Diisi';
				
				} else if($this->input->post('employee_id')==='') {
		        	$Return['error'] = 'Nama Karyawan Wajib Diisi';
				
		        } else if($this->input->post('category_id')==='') {
        			$Return['error'] = 'Kategori Aset Wajib Diisi';

        		} else if($this->input->post('assets_id')==='') {
        			$Return['error'] = 'Master Aset Wajib Diisi';

        		} else if($this->input->post('pinjam_date')==='') {
        			$Return['error'] = 'Tanggal Pinjam Aset Wajib Diisi';

        		// } else if($this->input->post('kembali_date')==='') {
        		// 	$Return['error'] = 'Tanggal Kembali Aset Wajib Diisi';

        		} else if($this->input->post('asset_note')==='') {
        			$Return['error'] = 'Catatan Aset Wajib Diisi';

        		} else if($this->input->post('is_pinjam')==='') {
        			$Return['error'] = 'Status Aset Wajib Diisi';

				} 
								
				if($Return['error']!=''){
		       		$this->output($Return);
		    	}

		    	$asset_id = $this->input->post('assets_id');
				
				$data_assets = array(
					'is_pinjam'    => 1					
				);
				
				$result = $this->Assets_model->update_assets_record($data_assets, $asset_id);
				// set data
				$data = array(
					'company_id'   => $this->input->post('company_id'),
					'employee_id'  => $this->input->post('employee_id'),
					'category_id'  => $this->input->post('category_id'),
					'assets_id'    => $this->input->post('assets_id'),
					'pinjam_date'  => $this->input->post('pinjam_date'),
					'kembali_date' => $this->input->post('kembali_date'),
					'asset_note'   => $this->input->post('asset_note'),
					'is_pinjam'    => $this->input->post('is_pinjam'),
					'created_at'   => date('d-m-Y h:i:s')
				);
				
				$result = $this->Assets_model->add_assets_pinjam($data);


				if ($result == TRUE) {
					$Return['result'] = 'Pinjam Asset Berhasil Ditambahkan';
				} else {
					$Return['error'] = $this->lang->line('xin_error_msg');
				}
				$this->output($Return);
				exit;
			}
		}
				
		public function update_assets_pinjam() 
		{
			if($this->input->post('edit_type')=='update_assets_pinjam') {		
			/* Define return | here result is used to return user data and error for error message */
			$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
				
			$id = $this->uri->segment(4);
			$Return['csrf_hash'] = $this->security->get_csrf_hash();
				
			/* Server side PHP input validation */		
			if($this->input->post('company_id')==='') {
	        	$Return['error'] = 'Nama Perusahaan Wajib Diisi';
			
			} else if($this->input->post('employee_id')==='') {
	        	$Return['error'] = 'Nama Karyawan Wajib Diisi';
			
	        } else if($this->input->post('category_id')==='') {
    			$Return['error'] = 'Kategori Aset Wajib Diisi';

    		} else if($this->input->post('assets_id')==='') {
    			$Return['error'] = 'Master Aset Wajib Diisi';

    		} else if($this->input->post('pinjam_date')==='') {
    			$Return['error'] = 'Tanggal Pinjam Aset Wajib Diisi';

    		// } else if($this->input->post('kembali_date')==='') {
    		// 	$Return['error'] = 'Tanggal Kembali Aset Wajib Diisi';

    		} else if($this->input->post('asset_note')==='') {
    			$Return['error'] = 'Catatan Aset Wajib Diisi';

    		} else if($this->input->post('is_pinjam')==='') {
    			$Return['error'] = 'Status Aset Wajib Diisi';

			} 
							
			if($Return['error']!=''){
	       		$this->output($Return);
	    	}
			
			$asset_id = $this->input->post('assets_id');

			$data_assets = array(
				'is_pinjam'    => 0				
			);
			
			$result = $this->Assets_model->update_assets_record($data_assets, $asset_id);
			// set data
			$data = array(
				'company_id'   => $this->input->post('company_id'),
				'employee_id'  => $this->input->post('employee_id'),
				'category_id'  => $this->input->post('category_id'),
				'assets_id'    => $this->input->post('assets_id'),
				'pinjam_date'  => $this->input->post('pinjam_date'),
				'kembali_date' => $this->input->post('kembali_date'),
				'asset_note'   => $this->input->post('asset_note'),
				'is_pinjam'    => $this->input->post('is_pinjam'),
				'created_at'   => date('d-m-Y h:i:s')
			);
			
			$result = $this->Assets_model->update_assets_pinjam_record($data,$id);
			if ($result == TRUE) {
				$Return['result'] = 'Pinjam Asset Berhasil Diperbaharui';
			} else {
				$Return['error'] = $this->lang->line('xin_error_msg');
			}
			$this->output($Return);
			exit;
			}
		}
	
		public function read_assets_pinjam() 
		{
			$session = $this->session->userdata('username');
			if(empty($session)){ 
				redirect('admin/');
			}
			$data['title'] = $this->Core_model->site_title();
			$id = $this->input->get('assets_category_id');
			$result = $this->Assets_model->read_assets_pinjam_info($id);
			$data = array(
					'assets_category_id' => $result[0]->assets_category_id,
					'company_id'         => $result[0]->company_id,
					'category_name'      => $result[0]->category_name,
					'created_at'         => $result[0]->created_at
					);
			if(!empty($session)){ 
				$this->load->view('admin/assets/dialog_assets_pinjam', $data);
			} else {
				redirect('admin/');
			}
		}

		public function delete_assets_pinjam() 
		{
			if($this->input->post('type')=='delete_record') {
				/* Define return | here result is used to return user data and error for error message */
				$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
				$id = $this->uri->segment(4);
				$Return['csrf_hash'] = $this->security->get_csrf_hash();
				$result = $this->Assets_model->delete_assets_pinjam_record($id);
				if(isset($id)) {
					$Return['result'] = 'Pinjam Asset Berhasil Dihapus';
				} else {
					$Return['error'] = $this->lang->line('xin_error_msg');
				}
				$this->output($Return);
			}
		}

	// =====================================================================================================	
    // KEMBALI ASET
    // ===================================================================================================== 
	
		public function kembali() 
		{
			$session = $this->session->userdata('username');
			if(empty($session)){
				redirect('admin/');
			}

			$data['title']         = 'Kembali | '.$this->Core_model->site_title();
			$data['desc']   	   = 'FORM INPUT : Proses Kembali Aset';
			$data['breadcrumbs']   = 'Kembali';
			$data['icon']          = '<i class="fa fa-mail-reply"></i>';			
			$data['path_url']      = 'assets_kembali';

			$data['all_companies']         = $this->Core_model->get_companies();
			$data['all_employees']         = $this->Core_model->all_employees();

			$data['all_assets_categories'] = $this->Assets_model->get_all_assets_categories();
			$data['all_assets']            = $this->Assets_model->get_all_assets();

			$role_resources_ids    = $this->Core_model->user_role_resource();
			if(in_array('27',$role_resources_ids)) {
				if(!empty($session)){ 
					$data['subview'] = $this->load->view("admin/assets/assets_kembali_list", $data, TRUE);
					$this->load->view('admin/layout/layout_main', $data); //page load
				} else {
					redirect('admin/');
				}
			} else {
				redirect('admin/dashboard');
			}
		}
		
		public function kembali_list()
	    {

			$session = $this->session->userdata('username');
			if(empty($session)){ 
				redirect('admin/');
			}
			$data['title'] = $this->Core_model->site_title();
			if(!empty($session)){ 
				$this->load->view("admin/assets/assets_kembali_list", $data);
			} else {
				redirect('admin/');
			}
			// Datatables Variables
			$draw = intval($this->input->get("draw"));
			$start = intval($this->input->get("start"));
			$length = intval($this->input->get("length"));
			
			
			$assets_kembali = $this->Assets_model->get_assets_kembali();
			$role_resources_ids = $this->Core_model->user_role_resource();
			$data = array();
				$no = 1;
	          foreach($assets_kembali->result() as $r) {	

	          	// get company
				$company = $this->Core_model->read_company_info($r->company_id);
				if(!is_null($company)){
					$comp_name = $company[0]->name;
				} else {
				 	$comp_name = '--';	
				}

				$user = $this->Core_model->read_user_info($r->employee_id);				
				if(!is_null($user)){
					$nip       = $user[0]->employee_id;
					$full_name = $user[0]->first_name.' '.$user[0]->last_name;
				} else {
					$nip       = '--';
					$full_name = '--';	
				}

				$asset_pinjam = $this->Core_model->read_asset_info($r->assets_id);				
				if(!is_null($user)){
					$asset_pinjam_name = $asset_pinjam[0]->name;
					$asset_pinjam_kode = $asset_pinjam[0]->company_asset_code;
				} else {
					$asset_pinjam_name = '--';
					$asset_pinjam_kode = '--';	
				}	

				
				if(in_array('279',$role_resources_ids)) { //view
					$view = '  <span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view').'">
									<button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light" data-toggle="modal" data-target=".view-modal-data" data-pinjam_id="'. $r->pinjam_id . '">
											<span class="fa fa-eye"></span> Lihat 
									</button>
								</span>';
				} else {
					$view = '';
				}

				$combhr = $view;

				if ($r->kembali_date == '0000-00-00'){
					$kembali = 'Belum Dikembalikan';
				} else {
					$kembali =  date("d-m-Y",strtotime($r->kembali_date));
				}

				if ($r->is_pinjam == '1'){
					$status = 'Dipinjam';
				} else {
					$status = 'Dikembalikan';
				}
										 			  				
				$data[] = array(
					$combhr,
					date("d-m-Y",strtotime($r->pinjam_date)),
					$kembali,
					$nip,
					$full_name,
					$asset_pinjam_kode,
					$asset_pinjam_name,
					$status
				);
			}
	          $output = array(
	               "draw" => $draw,
	                 "recordsTotal" => $assets_kembali->num_rows(),
	                 "recordsFiltered" => $assets_kembali->num_rows(),
	                 "data" => $data
	            );
	          echo json_encode($output);
	          exit();
	    }

	    public function asset_kembali_read()
		{
			$session = $this->session->userdata('username');
			if(empty($session)){ 
				redirect('admin/');
			}
			$data['title'] = $this->Core_model->site_title();
			$id            = $this->input->get('pinjam_id');
			$result        = $this->Assets_model->read_assets_pinjam_info($id);
			$data = array(

					'pinjam_id'     => $result[0]->pinjam_id,

					'company_id'     => $result[0]->company_id,
				    'employee_id'    => $result[0]->employee_id,
					
					'category_id'   => $result[0]->category_id,
					'assets_id'     => $result[0]->assets_id,
					
					'pinjam_date'   => $result[0]->pinjam_date,
					'kembali_date'  => $result[0]->kembali_date,

					'is_pinjam'     => $result[0]->is_pinjam,

					'asset_note'    => $result[0]->asset_note,
					
					'created_at'    => $result[0]->created_at,
					
					'all_companies'  => $this->Core_model->get_companies(),
					'all_employees'  => $this->Core_model->all_employees(),
					'all_categories' => $this->Assets_model->get_all_assets_categories(),
					'all_assets'     => $this->Assets_model->get_all_assets()
					
			);
			if(!empty($session)){ 
				$this->load->view('admin/assets/dialog_asset_kembali', $data);
			} else {
				redirect('admin/');
			}
		}
	   
		public function add_asset_kembali() 
		{
			if($this->input->post('add_type')=='add_asset_kembali') {		
				/* Define return | here result is used to return user data and error for error message */
				$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
				$Return['csrf_hash'] = $this->security->get_csrf_hash();
					
				/* Server side PHP input validation */		
				if($this->input->post('company_id')==='') {
		        	$Return['error'] = 'Nama Perusahaan Wajib Diisi';
				
				} else if($this->input->post('employee_id')==='') {
		        	$Return['error'] = 'Nama Karyawan Wajib Diisi';
				
		        } else if($this->input->post('category_id')==='') {
        			$Return['error'] = 'Kategori Aset Wajib Diisi';

        		} else if($this->input->post('assets_id')==='') {
        			$Return['error'] = 'Master Aset Wajib Diisi';

        		} else if($this->input->post('pinjam_date')==='') {
        			$Return['error'] = 'Tanggal Pinjam Aset Wajib Diisi';

        		// } else if($this->input->post('kembali_date')==='') {
        		// 	$Return['error'] = 'Tanggal Kembali Aset Wajib Diisi';

        		} else if($this->input->post('asset_note')==='') {
        			$Return['error'] = 'Catatan Aset Wajib Diisi';

        		} else if($this->input->post('is_pinjam')==='') {
        			$Return['error'] = 'Status Aset Wajib Diisi';

				} 
								
				if($Return['error']!=''){
		       		$this->output($Return);
		    	}
				
				// set data
				$data = array(
					'company_id'   => $this->input->post('company_id'),
					'employee_id'  => $this->input->post('employee_id'),
					'category_id'  => $this->input->post('category_id'),
					'assets_id'    => $this->input->post('assets_id'),
					'pinjam_date'  => $this->input->post('pinjam_date'),
					'kembali_date' => $this->input->post('kembali_date'),
					'asset_note'   => $this->input->post('asset_note'),
					'is_pinjam'    => $this->input->post('is_pinjam'),
					'created_at'   => date('d-m-Y h:i:s')
				);
				
				$result = $this->Assets_model->add_assets_pinjam($data);
				if ($result == TRUE) {
					$Return['result'] = 'Kembali Asset Berhasil Ditambahkan';
				} else {
					$Return['error'] = $this->lang->line('xin_error_msg');
				}
				$this->output($Return);
				exit;
			}
		}
				
		public function update_assets_kembali() 
		{
			if($this->input->post('edit_type')=='update_assets_kembali') {		
			/* Define return | here result is used to return user data and error for error message */
			$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
				
			$id = $this->uri->segment(4);
			$Return['csrf_hash'] = $this->security->get_csrf_hash();
				
			/* Server side PHP input validation */		
			if($this->input->post('company_id')==='') {
	        	$Return['error'] = 'Nama Perusahaan Wajib Diisi';
			
			} else if($this->input->post('employee_id')==='') {
	        	$Return['error'] = 'Nama Karyawan Wajib Diisi';
			
	        } else if($this->input->post('category_id')==='') {
    			$Return['error'] = 'Kategori Aset Wajib Diisi';

    		} else if($this->input->post('assets_id')==='') {
    			$Return['error'] = 'Master Aset Wajib Diisi';

    		} else if($this->input->post('pinjam_date')==='') {
    			$Return['error'] = 'Tanggal Pinjam Aset Wajib Diisi';

    		// } else if($this->input->post('kembali_date')==='') {
    		// 	$Return['error'] = 'Tanggal Kembali Aset Wajib Diisi';

    		} else if($this->input->post('asset_note')==='') {
    			$Return['error'] = 'Catatan Aset Wajib Diisi';

    		} else if($this->input->post('is_pinjam')==='') {
    			$Return['error'] = 'Status Aset Wajib Diisi';

			} 
							
			if($Return['error']!=''){
	       		$this->output($Return);
	    	}
			
			// set data
			$data = array(
				'company_id'   => $this->input->post('company_id'),
				'employee_id'  => $this->input->post('employee_id'),
				'category_id'  => $this->input->post('category_id'),
				'assets_id'    => $this->input->post('assets_id'),
				'pinjam_date'  => $this->input->post('pinjam_date'),
				'kembali_date' => $this->input->post('kembali_date'),
				'asset_note'   => $this->input->post('asset_note'),
				'is_pinjam'    => $this->input->post('is_pinjam'),
				'created_at'   => date('d-m-Y h:i:s')
			);
			
			$result = $this->Assets_model->update_assets_pinjam_record($data,$id);
			if ($result == TRUE) {
				$Return['result'] = 'Kembali Asset Berhasil Diperbaharui';
			} else {
				$Return['error'] = $this->lang->line('xin_error_msg');
			}
			$this->output($Return);
			exit;
			}
		}
	
		public function read_assets_kembali() 
		{
			$session = $this->session->userdata('username');
			if(empty($session)){ 
				redirect('admin/');
			}
			$data['title'] = $this->Core_model->site_title();
			$id = $this->input->get('assets_category_id');
			$result = $this->Assets_model->read_assets_pinjam_info($id);
			$data = array(
					'assets_category_id' => $result[0]->assets_category_id,
					'company_id'         => $result[0]->company_id,
					'category_name'      => $result[0]->category_name,
					'created_at'         => $result[0]->created_at
					);
			if(!empty($session)){ 
				$this->load->view('admin/assets/dialog_assets_kembali', $data);
			} else {
				redirect('admin/');
			}
		}

		public function delete_assets_kembali() 
		{
			if($this->input->post('type')=='delete_record') {
				/* Define return | here result is used to return user data and error for error message */
				$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
				$id = $this->uri->segment(4);
				$Return['csrf_hash'] = $this->security->get_csrf_hash();
				$result = $this->Assets_model->delete_assets_pinjam_record($id);
				if(isset($id)) {
					$Return['result'] = 'Kembali Asset Berhasil Dihapus';
				} else {
					$Return['error'] = $this->lang->line('xin_error_msg');
				}
				$this->output($Return);
			}
		}

	// =====================================================================================================	
    // LAIN
    // =====================================================================================================
	 
		// get company > employees
		public function get_employees() 
		{
			$data['title'] = $this->Core_model->site_title();
			$id = $this->uri->segment(4);
			
			$data = array(
				'company_id' => $id
				);
			$session = $this->session->userdata('username');
			if(!empty($session)){ 
				$this->load->view("admin/assets/get_employees", $data);
			} else {
				redirect('admin/');
			}
			// Datatables Variables
			$draw = intval($this->input->get("draw"));
			$start = intval($this->input->get("start"));
			$length = intval($this->input->get("length"));
		}

		// get company > employees
		public function get_assets() 
		{
			$data['title'] = $this->Core_model->site_title();
			$id = $this->uri->segment(4);
			
			$data = array(
				'assets_category_id' => $id
			);
			
			$session = $this->session->userdata('username');
			if(!empty($session)){ 
				$this->load->view("admin/assets/get_assets", $data);
			} else {
				redirect('admin/');
			}
			// Datatables Variables
			$draw = intval($this->input->get("draw"));
			$start = intval($this->input->get("start"));
			$length = intval($this->input->get("length"));
		}
	
} 
?>