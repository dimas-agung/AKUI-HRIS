...
class Employees extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['title'] = $this->lang->line('xin_employees').' | '.$this->Core_model->site_title();
	...
	public function hr() {
		...
		$data['title'] = $this->lang->line('xin_employees_directory').' | '.$this->Core_model->site_title();
		$data['all_departments'] = $this->Department_model->all_departments();
		$data['all_designations'] = $this->Designation_model->all_designations();
		$data['all_user_roles'] = $this->Roles_model->all_user_roles();
	...
	public function document_info() {
		...
		} else if($this->Core_model->validate_date($this->input->post('date_of_expiry'),'Y-m-d') == false) {
			 $Return['error'] = $this->lang->line('xin_hr_date_format_error');
		...
		//clean simple fields
		$title = $this->Core_model->clean_post($this->input->post('title'));
		$description = $this->Core_model->clean_post($this->input->post('description'));
		// clean date fields
		$date_of_expiry = $this->Core_model->clean_date_post($this->input->post('date_of_expiry'));
	...
	public function immigration_info() {
		...
		} else if($this->Core_model->validate_date($this->input->post('issue_date'),'Y-m-d') == false) {
			 $Return['error'] = $this->lang->line('xin_hr_date_format_error');
		} else if($this->input->post('expiry_date')==='') {
			$Return['error'] = $this->lang->line('xin_employee_error_expiry_date');
		} else if($this->Core_model->validate_date($this->input->post('expiry_date'),'Y-m-d') == false) {
			 $Return['error'] = $this->lang->line('xin_hr_date_format_error');
		}
		...
		$document_number = $this->Core_model->clean_post($this->input->post('document_number'));	
		$issue_date = $this->Core_model->clean_date_post($this->input->post('issue_date'));
		$expiry_date = $this->Core_model->clean_date_post($this->input->post('expiry_date'));
		$eligible_review_date = $this->Core_model->clean_date_post($this->input->post('eligible_review_date'));
		$data = array(
		'document_type_id' => $this->input->post('document_type_id'),
	...
	public function salary_all_allowances() {
		...
		$default_currency = $this->Core_model->read_currency_con_info($system[0]->default_currency_id);
