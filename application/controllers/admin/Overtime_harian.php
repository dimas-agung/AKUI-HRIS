...
class overtime_harian extends MY_Controller {
	...
	// =================================================================================================================
		...
		public function index()
	    {
			...
			$data['title']       = 'Lembur Harian | '.$this->Core_model->site_title();
			$data['icon']        = '<i class="fa fa-tasks"></i>';
			$data['desc']        = '<b>INFORMASI : </b> Proses Lembur harian Karyawan';
			$data['breadcrumbs'] = 'Lembur Harian';
			$data['path_url']    = 'overtime_harian';
			...
			if(in_array('0691',$role_resources_ids)) {
				if(!empty($session)){ 
					$data['subview'] = $this->load->view("admin/overtime/overtime_list_harian", $data, TRUE);
	    ...
	    public function overtime_list_harian() 
	    {
			$data['title'] = $this->Core_model->site_title();
			$session = $this->session->userdata('username');
			if(!empty($session)){ 
				$this->load->view("admin/overtime/overtime_list_harian", $data);
			} else {
				redirect('admin/');
			}
			...
			$overtime = $this->Overtime_model->get_overtime_harian();
	        ...
	        foreach($overtime->result() as $r) {
				...
				if(in_array('0693',$role_resources_ids)) { //edit
				...
				if(in_array('0694',$role_resources_ids)) { // delete
				...
				if(in_array('0695',$role_resources_ids)) { //view
					$view = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view_details').'"><a target="_blank" href="'.site_url().'admin/overtime_harian/details_harian/'.$r->overtime_id.'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-eye"></span></button></a></span> ';
				} else {
					$view = '';
				}
		...
		public function get_employees_harian() 
		{
			...
			$session = $this->session->userdata('username');
			if(!empty($session)){ 
				$this->load->view("admin/overtime/get_employees_harian", $data);
			} else {
				redirect('admin/');
			}
			// Datatables Variables
		...
		public function add_overtime_harian() 
		{
		
			if($this->input->post('add_type')=='overtime') {	
				...
				foreach(explode(',',$employee_id) as $uid) {
					...
					if(!is_null($user)){
						...
						// ===================================================================================
							...
							$jumlah_upah         = $jumlah_basic_salary;
						...
						// ===================================================================================
							...
							$hitung_biaya         = ($jumlah_upah/7.5*1.5)*$jam_1;
							$biaya_lembur_jam_1   = round($hitung_biaya,2);

							$jam_2                = 0 ; 
							$biaya_lembur_jam_2   = 0;
		...
		public function read_harian()
		{
			...
			$session = $this->session->userdata('username');
			if(!empty($session)){ 
				$this->load->view('admin/overtime/dialog_overtime_harian', $data);
			} else {
				redirect('admin/');
			}
		}	
		...
		// overtime details
		public function details_harian()
	    {
			$session = $this->session->userdata('username');
			if(empty($session)){ 
				redirect('admin/');
			...
			$data = array(
					...
					'title' 			=> "Detail Lembur Harian | ". $this->Core_model->site_title(),
					'icon'      		=> "<i class='fa fa-eye'></i>",
					'breadcrumbs' 		=> "Detail Lembur Harian",
					'path_url' 			=> 'overtime_details_harian',
			...
			// $data['breadcrumbs'] = $this->lang->line('xin_overtime_details');
			
			// $data['path_url'] = 'overtime_details';
			
			$role_resources_ids = $this->Core_model->user_role_resource();
			
			if(in_array('0695',$role_resources_ids)) {
				if(!empty($session)){ 
					$data['subview'] = $this->load->view("admin/overtime/overtime_details_harian", $data, TRUE);
					$this->load->view('admin/layout/layout_main', $data); //page load
				} else {
					redirect('admin/');
				}
		 ...
		 // Validate and update info in database
		public function update_status_harian() 
		{
		
			if($this->input->post('edit_type')=='update_status_harian') {
