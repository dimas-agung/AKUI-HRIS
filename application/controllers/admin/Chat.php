...
class Chat extends MY_Controller
{
   ...
   /*Function to set JSON output*/
	...
	public function index() {
		...
		$data['title'] = $this->lang->line('xin_hr_chat_box').' | '.$this->Core_model->site_title();
		$data['breadcrumbs'] = $this->lang->line('xin_hr_chat_box');
		$data['path_url'] = 'chatbox';
		//$data['all_active_employees'] = $this->Core_model->all_active_employees();
			  ...
			  <div class="chats" id="chatbox">';
				foreach($this->Chat_model->get_messages() as $msgs){
					...
					if($session['user_id']!=$msgs->from_id){
						$user_info = $this->Core_model->read_user_info($msgs->from_id);
						//send
                      ...
                      <div class="flex-shrink-1 bg-lighter rounded py-2 px-3 mr-3">
                        ...
                        '.$msgs->message_content.'
							...
							$fuser_info = $this->Core_model->read_user_info($msgs->from_id);
							//from
		  ...
		  $("#xin-form").submit(function(e){
			...
			$.ajax({
				...
				success: function(JSON)
				{
					...
					$("#message_content").val("");
					$("input[name="csrf_hris"]").val(data.csrf_hash);
	...
	public function refresh_chat_users_msg() {
		...
		$unread_msgs = $this->Core_model->get_single_unread_message($session['user_id']);
		if($unread_msgs > 0) {
			echo $unread_msgs;
		} else {
			echo '';
	...
	public function refresh_chat_users() {
		...
		$fuser_info = $this->Core_model->read_user_info($session['user_id']);
		if($fuser_info[0]->user_role_id==1){
			$all_active_employees = $this->Core_model->all_active_employees();
		} else {
			$all_active_employees = $this->Core_model->all_active_departments_employees();
		}
