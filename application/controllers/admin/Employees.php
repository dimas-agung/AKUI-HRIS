...
class Employees extends MY_Controller {
	...
	public function index()
    {
		...
		if(in_array('132',$role_resources_ids) || $reports_to > 0) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/employees/employees_active", $data, TRUE);
	...
	public function employees_list()
    {
        ...
        foreach($employee->result() as $r) {		  
			...
			if(in_array('202',$role_resources_ids)) {
				$kontrak_opt = ' <span data-toggle="tooltip" data-placement="top" title="Kontrak Karyawan"><a target="_blank" href="'.site_url().'admin/employees/kontrak/'.$r->user_id.'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-file"></span></button></a></span>';
			} else {
				$kontrak_opt = '';
			}

			if(in_array('202',$role_resources_ids)) {
				$edit_opt = ' <span data-toggle="tooltip" data-placement="top" title="Edit Karyawan"><a target="_blank" href="'.site_url().'admin/employees/detail/'.$r->user_id.'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-pencil"></span></button></a></span>';
			} else {
				$edit_opt = '';
			}
	...
	// =================================================================
	    ...
	    public function employees_list_active()
	    {
			...
			$session = $this->session->userdata('username');
			if(!empty($session)){ 
				$this->load->view("admin/employees/employees_active", $data);
			} else {
				redirect('admin/');
			}
			// Datatables Variables
