...
class Promotion extends MY_Controller {
	...
	// =================================================================================================================
		...
		public function index()
	    {
			...
			$data['icon']        = '<i class="fa fa-cloud-upload"></i>';
			$data['desc']        = 'INFORMASI : Proses Personalia - Promosi Jabatan / Posisi Karyawan';
			...
			if(in_array('0661',$role_resources_ids)) {
				if(!empty($session)){ 
	    ...
	    public function promotion_list()
	    {
	        ...
	        foreach($promotion->result() as $r) {
				...
				if(in_array('0663',$role_resources_ids)) { //edit
					$edit = '<span data-toggle="tooltip" data-placement="top" title="Aktifkan">
								<button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-promotion_id="'. $r->promotion_id . '">
				...
				if(in_array('0664',$role_resources_ids)) { // delete
					$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'">
									<button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->promotion_id . '">
				...
				if(in_array('0665',$role_resources_ids)) { //view
	...
	// =================================================================================================================
		...
		public function view() 
	    {
			...
			if ( $result[0]->title == '1'){
					...
					$jenis_gaji = 'Gaji Bulanan';

				} else if ( $result[0]->title == '2') {

					$jenis_gaji = 'Gaji Harian';

				} else if ( $result[0]->title == '3'){

					$jenis_gaji = 'Gaji Borongan';
			...
			$data = array(
				...
				$session = $this->session->userdata('username');
			if(!empty($session)){ 
				$this->load->view('admin/promotion/dialog_promotion_view', $data);
			} else {
				redirect('admin/');
			}
		}
