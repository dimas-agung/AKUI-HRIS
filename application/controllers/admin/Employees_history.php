...
class Employees_history extends MY_Controller {
	...
	public function index()
    {
		...
		$data['title']                = 'Karyawan History | '.$this->Core_model->site_title();
		$data['icon']                 = '<i class="fa fa-sign-out"></i>';
		$data['desc']                 = 'INFORMASI : Daftar Karyawan Bekerja saat ini';
		$data['breadcrumbs']          = 'Karyawan History';
		$data['path_url']             = 'employees_history';
		...
		if(in_array('0511',$role_resources_ids) || $reports_to > 0) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/employees_history/employees_history", $data, TRUE);
	...
	public function employees_list_history()
    {

		$data['title'] = $this->Core_model->site_title();
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("admin/employees_history/employees_history", $data);
		} else {
			redirect('admin/');
		}
		...
		if($this->input->get("ihr")=='true'){
			if($this->input->get("company_id")==0 && $this->input->get("location_id")==0 && $this->input->get("department_id")==0 && $this->input->get("designation_id")==0){
				$employee = $this->Employees_model->get_employees_history();
				...
				$employee = $this->Employees_model->get_company_employees_flt_all($this->input->get("company_id"));
			
			} else if($this->input->get("company_id")!=0 && $this->input->get("location_id")!=0 && $this->input->get("department_id")==0 && $this->input->get("designation_id")==0){
				$employee = $this->Employees_model->get_company_location_employees_flt_all($this->input->get("company_id"),$this->input->get("location_id"));
				
			} else if($this->input->get("company_id")!=0 && $this->input->get("location_id")!=0 && $this->input->get("department_id")!=0 && $this->input->get("designation_id")==0){
				$employee = $this->Employees_model->get_company_location_department_employees_history($this->input->get("company_id"),$this->input->get("location_id"),$this->input->get("department_id"));
				
			} else if($this->input->get("company_id")!=0 && $this->input->get("location_id")!=0 && $this->input->get("department_id")!=0 && $this->input->get("designation_id")!=0){
				$employee = $this->Employees_model->get_company_location_department_designation_employees_flt_all($this->input->get("company_id"),$this->input->get("location_id"),$this->input->get("department_id"),$this->input->get("designation_id"));
			}
		} else {	
        ...
        foreach($employee->result() as $r) {		  
			...
			$employee_date = date("d-m-Y", strtotime($r->date_of_joining)).' '.$status.'<br> 
			                 <small class="text-muted">Tgl Lhr : '.date("d-m-Y", strtotime($r->date_of_birth)).' <br>Usia : '.$selisih.' </small>';
