...
class Employees_resign extends MY_Controller {
	...
	public function index()
    {
		...
		$data['icon']                 = '<i class="fa fa-thumb-tack"></i>';
		$data['breadcrumbs']          = 'Karyawan Resign';
		$data['path_url']             = 'employees_resign';
		...
		if(in_array('0520',$role_resources_ids) || $reports_to > 0) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/employees_resign/employees_resign", $data, TRUE);
	...
	public function employees_resign_list()
    {

		$data['title'] = $this->Core_model->site_title();
		$session = $this->session->userdata('username');
		if(!empty($session)){ 
			$this->load->view("admin/employees_resign/employees_resign", $data);
		} else {
			redirect('admin/');
		}
		...
		$employee = $this->Employees_model->get_employees_resign();
        ...
        foreach($employee->result() as $r) {		  
			...
			if(in_array('202',$role_resources_ids)) {
				$kontrak_opt = ' <span data-toggle="tooltip" data-placement="top" title="Kontrak Karyawan"><a target="_blank" href="'.site_url().'admin/employees_resign/kontrak/'.$r->user_id.'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-file"></span></button></a></span>';
			} else {
				$kontrak_opt = '';
			}

			if(in_array('202',$role_resources_ids)) {
				$edit_opt = ' <span data-toggle="tooltip" data-placement="top" title="Edit Karyawan"><a target="_blank" href="'.site_url().'admin/employees_resign/detail/'.$r->user_id.'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-pencil"></span></button></a></span>';
			} else {
				$edit_opt = '';
			}

			if(in_array('421',$role_resources_ids)) {
				$down_opt = ' <span data-toggle="tooltip" data-placement="top" title="Hapus Karyawan"><a target="_blank" href="'.site_url().'admin/employees/download_profile/'.$r->user_id.'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-download"></span></button></a></span>';
			} else {
				$down_opt = '';
			}
			
			$function = $kontrak_opt.' '.$edit_opt;
			...
			$employee_date_join = date("d M Y", strtotime($r->date_of_joining));

			$employee_date_resign = date("d M Y", strtotime($r->exit_date));
	...
	public function view() 
	{
		...
		$data['subview'] = $this->load->view("admin/employees/employee_view", $data, TRUE);
		$this->load->view('admin/layout/layout_main', $data); //page load
	...
	public function add_employee() 
	{
		...
		$module_attributes = $this->Custom_fields_model->all_hris_module_attributes();
		...
		if ($iresult) {
			...
			if($count_module_attributes > 0){
				foreach($module_attributes as $mattribute) {
					...
					/*$attr_orig_value = $this->Custom_fields_model->read_hris_module_attributes_values($result,$mattribute->custom_field_id);
			...
			$company = $this->Core_model->read_company_setting_info(1);
			if($setting[0]->enable_email_notification == 'yes') {
				...
				//get email template
				$template = $this->Core_model->read_email_template(8);
				...
				hris_mail($cinfo[0]->email,$cinfo[0]->company_name,$this->input->post('email'),$subject,$message);				
			}
    ...
    public function employees_list_all()
    {
        ...
        foreach($employee->result() as $r) {		  
			...
			if(in_array('202',$role_resources_ids)) {
				$view_opt = ' <span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view_details').'"><a href="'.site_url().'admin/employees/view/'.$r->user_id.'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-eye"></span> Lihat </button></a></span> ';
			} else {
				$view_opt = '';
			}

			if(in_array('421',$role_resources_ids)) {
				$down_opt = ' <span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_download_profile_title').'"><a target="_blank" href="'.site_url().'admin/employees/download_profile/'.$r->user_id.'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-download"></span></button></a></span>';
			} else {
				$down_opt = '';
			}
			// if(in_array('421',$role_resources_ids)) {
			// 	$employee_name .= '<br><small class="text-muted "><a target="_blank" href="'.site_url('admin/employees/download_profile/').$r->user_id.'">'.$this->lang->line('xin_download_profile_title').' <i class="fa fa-eye"></i></a></small>';
			// }
			$function = $view_opt;
	...
	public function myteam_employees_list()
    {
        ...
        foreach($employee->result() as $r) {		  
			...
			if(in_array('202',$role_resources_ids)) {
				$view_opt = ' <span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_view_details').'"><a href="'.site_url().'admin/employees/detail/'.$r->user_id.'"><button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"><span class="fa fa-eye"></span></button></a></span>';
			} else {
				$view_opt = '';
			}
			...
			if(in_array('421',$role_resources_ids)) {
				$employee_name .= $full_name.'<br><small class="text-muted "><a target="_blank" href="'.site_url('admin/employees/download_profile/').$r->user_id.'">'.$this->lang->line('xin_download_profile_title').' <i class="fa fa-eye"></i></a></small>';
			}
 ...
 //    {
	...
	// 	$employee = $this->Employees_model->get_employees();
	// 	$countries = $this->Core_model->get_countries();
 ...
 //    }		
	...
	// ==================================================================================== 
		public function kontrak() 
		{
			...
			$data['subview'] = $this->load->view("admin/employees/employee_kontrak", $data, TRUE);
			$this->load->view('admin/layout/layout_main', $data); //page load
