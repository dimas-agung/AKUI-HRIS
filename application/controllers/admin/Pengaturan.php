...
class Pengaturan extends MY_Controller
{
    ...
    // Jam Reguler
    public function office_reguler()
    {
        ...
        $data['title']             = 'Atur Jadwal Reguler | ' . $this->Core_model->site_title();
        $data['icon']              = '<i class="fa fa-share-alt"></i>';
        $data['breadcrumbs']       = 'Atur Jadwal Reguler';
        $data['path_url']          = 'pengaturan_office_reguler';
        ...
        if (in_array('0810', $role_resources_ids)) {
            if (!empty($session)) {
                $data['subview'] = $this->load->view("admin/pengaturan/office_reguler", $data, TRUE);
    ...
    // Jam Reguler > Daftar
    public function office_reguler_list()
    {
        $data['title'] = $this->Core_model->site_title();
        $session = $this->session->userdata('username');
        if (!empty($session)) {
            $this->load->view("admin/pengaturan/office_reguler", $data);
        } else {
            redirect('admin/');
        }
        ...
        $office_shift = $this->Timesheet_model->get_office_reguler();
        ...
        foreach ($office_shift->result() as $r) {
            ...
            if ($r->monday_in_time == '') {
                $monday = '<span class="blink blink-tree merah"> Libur </span>';
            } else {
                $monday = $monday_in_time->format('H:i:s') . '<br>' . $this->lang->line('dashboard_to') . '<br>' . $monday_out_time->format('H:i:s');
            }
            if ($r->tuesday_in_time == '') {
                $tuesday = '<span class="blink blink-tree merah"> Libur </span>';
            } else {
                $tuesday = $tuesday_in_time->format('H:i:s') . '<br>' . $this->lang->line('dashboard_to') . '<br>' . $tuesday_out_time->format('H:i:s');
            }
            if ($r->wednesday_in_time == '') {
                $wednesday = '<span class="blink blink-tree merah"> Libur </span>';
            } else {
                $wednesday = $wednesday_in_time->format('H:i:s') . '<br>' . $this->lang->line('dashboard_to') . '<br>' . $wednesday_out_time->format('H:i:s');
            }
            if ($r->thursday_in_time == '') {
                $thursday = '<span class="blink blink-tree merah"> Libur </span>';
            } else {
                $thursday = $thursday_in_time->format('H:i:s') . '<br>' . $this->lang->line('dashboard_to') . '<br>' . $thursday_out_time->format('H:i:s');
            }
            if ($r->friday_in_time == '') {
                $friday = '<span class="blink blink-tree merah"> Libur </span>';
            } else {
                $friday = $friday_in_time->format('H:i:s') . '<br>' . $this->lang->line('dashboard_to') . '<br>' . $friday_out_time->format('H:i:s');
            }
            if ($r->saturday_in_time == '') {
                $saturday = '<span class="blink blink-tree merah"> Libur </span>';
            } else {
                $saturday = $saturday_in_time->format('H:i:s') . '<br>' . $this->lang->line('dashboard_to') . '<br>' . $saturday_out_time->format('H:i:s');
            }
            if ($r->sunday_in_time == '') {
                $sunday = '<span class="blink blink-tree merah"> Libur </span>';
            } else {
                $sunday = $sunday_in_time->format('H:i:s') . '<br>' . $this->lang->line('dashboard_to') . '<br>' . $sunday_out_time->format('H:i:s');
            }
            ...
            if (!is_null($cek_payroll_id)) {
                $payroll_month = $cek_payroll_id[0]->desc . '<br><small>Periode Kehadiran : <br>' . date("d-m-Y", strtotime($cek_payroll_id[0]->start_date)) . ' s/d ' . date("d-m-Y", strtotime($cek_payroll_id[0]->end_date)) . '</small>';
            } else {
                $payroll_month = '--';
            }
            ...
            if (!is_null($company)) {
                $comp_name = '<span class="badge bg-green">' . $company[0]->name . '</span>';
            } else {
                $comp_name = '--';
            }
            ...
            if (in_array('0813', $role_resources_ids)) { //di
            ...
            if (in_array('0814', $role_resources_ids)) { // delete
            ...
            if ($r->office_shift_id == '') {
                $ol = '<span class="blink blink-one"> 0 Karyawan </span>';
            } else {
                $ol = '<ol class="nl">';
                $employee = $this->Core_model->read_employee_info_pola($r->office_shift_id);
                foreach ($employee->result() as $e) {
                    ...
                    if (!is_null($employee)) {
                        $ol .= '<li>' . $e->first_name . ' ' . $e->last_name . ' (' . $designation_name . ')</li>';
                    } else {
                        $ol .= '--';
                    }
                }
            ...
            $data[] = array(
                $combhr,
                // $payroll_month,
                $r->shift_name . '<br><i class="fa fa-building"></i> <small> Perusahaan : <b>' . $comp_name . ' </b> </small>
                                    <br><i class="fa fa-flag"></i> <small> Jenis Pola Kerja : <b>' . $jenis_pola . ' </b></small>
                                    <br><i class="fa fa-users"></i> <small> Pola Kerja ini terdiri atas karyawan : <br>' . $ol . '</small>',
                //  . '<br>' .$jam,
                $monday,
                $tuesday,
                $wednesday,
