...
class User extends MY_Controller {
	...
	public function promotion()
     {
		...
		$data['title'] = $this->lang->line('left_promotions').' | '.$this->Core_model->site_title();
		$data['all_employees'] = $this->Core_model->all_employees();
	...
	public function complaints()
     {
		...
		$data['title'] = $this->lang->line('left_complaints').' | '.$this->Core_model->site_title();
		$data['all_employees'] = $this->Core_model->all_employees();
	...
	public function training()
     {
		...
		$data['title'] = $this->lang->line('left_training').' | '.$this->Core_model->site_title();
		$data['all_employees'] = $this->Core_model->all_employees();
	...
	public function training_details()
     {
		...
		$data['title'] = $this->Core_model->site_title().' | '.$this->Core_model->site_title();
		...
		$data = array(
				'title' => $this->lang->line('xin_training_details').' | '.$this->Core_model->site_title(),
	...
	public function office_shift()
     {
		...
		$data['title'] = $this->lang->line('left_office_shift').' | '.$this->Core_model->site_title();
		$data['all_employees'] = $this->Core_model->all_employees();
    ...
    public function office_shift_list()
     {
		...
		$user = $this->Core_model->get_employee_row($session['user_id']);
      ...
      //  foreach($user->result() as $r) {
		...
		// status info
		$shift = $this->Core_model->get_employee_shift_office($user[0]->office_shift_id);
		if(!is_null($shift)){
	...
	public function performance()
     {
		...
		$data['title'] = $this->lang->line('left_performance').' | '.$this->Core_model->site_title();
		$data['all_employees'] = $this->Core_model->all_employees();
	...
	// date wise date_wise_attendance > employee > attendance
	 public function attendance()
     {
		...
		$data['title'] = $this->lang->line('dashboard_attendance').' | '.$this->Core_model->site_title();
		$data['all_employees'] = $this->Core_model->all_employees();
	...
	public function add_leave() {
		...
		$user = $this->Core_model->read_user_info($this->input->post('employee_id'));
		if(!is_null($user)){
		...
		if ($result == TRUE) {
			...
			if($setting[0]->enable_email_notification == 'yes') {
				...
				//get company info
				$cinfo = $this->Core_model->read_company_setting_info(1);
				//get email template
				$template = $this->Core_model->read_email_template(5);
				//get employee info
				$user_info = $this->Core_model->read_user_info($this->input->post('employee_id'));
	...
	// leave > timesheet
	 public function leave_details() {
		...
		$data['title'] = $this->lang->line('xin_leave_detail').' | '.$this->Core_model->site_title();
		...
		$data = array(
				'title' => $this->lang->line('xin_leave_detail').' | '.$this->Core_model->site_title(),
				'type' => $type_name,
				'role_id' => $u_role_id,
				'full_name' => $full_name,
	...
	public function tickets()
     {
		...
		$data['title'] = $this->lang->line('left_tickets').' | '.$this->Core_model->site_title();
		$data['all_employees'] = $this->Core_model->all_employees();
		$data['all_companies'] = $this->Company_model->get_company();
    ...
    public function ticket_list()
     {
		...
		$p_company = $this->Core_model->read_company_info($r->company_id);
	 ...
	 public function comments_list()
     {
		...
		$id = $this->uri->segment(4);
		$session = $this->session->userdata('username');
		$ses_user = $this->Core_model->read_user_info($session['user_id']);
		if(!empty($session)){ 
			$this->load->view("employee/user/ticket_list", $data);
		} else {
			redirect('admin/');
		...
		// get user > employee_
		$employee = $this->Core_model->read_user_info($r->user_id);
		// employee full name
		if(!is_null($employee)){
		$employee_name = $employee[0]->first_name.' '.$employee[0]->last_name;
		// get designation
		...
		$created_at = date('h:i A', strtotime($r->created_at));
		$_date = explode(' ',$r->created_at);
		$date = $this->Core_model->set_date_format($_date[0]);
		///
	...
	public function add_ticket() {
		...
		$ticket_code = $this->Core_model->generate_random_string();
	...
	public function ticket_details()
     {
		...
		$data = array(
				'title' => $this->lang->line('xin_ticket_details').' | '.$this->Core_model->site_title(),
	...
	public function tasks() {
		...
		$data['title'] = $this->lang->line('left_tasks').' | '.$this->Core_model->site_title();
		$data['all_employees'] = $this->Core_model->all_employees();
	 ...
	 public function task_details() {
		...
		$data['title'] = $this->lang->line('xin_task_detail').' | '.$this->Core_model->site_title();
		...
		/* get User info*/
		$u_created = $this->Core_model->read_user_info($result[0]->created_by);
		...
		$data = array(
		'title' => $this->lang->line('xin_task_detail').' | '.$this->Core_model->site_title(),
	 ...
	 public function task_list() {
		...
		$data = array();
          ...
          foreach($task->result() as $r) {
			...
			//$ol = 'A';
			/* get User info*/
			$u_created = $this->Core_model->read_user_info($r->created_by);
			if(!is_null($u_created)){
				$f_name = $u_created[0]->first_name.' '.$u_created[0]->last_name;
			} else {
				$f_name = '--';	
	...
	/* Projects *//////////////////////////////////////////
	 public function projects()
     {
		...
		$data['title'] = $this->lang->line('xin_projects').' | '.$this->Core_model->site_title();
		$data['all_employees'] = $this->Core_model->all_employees();
		$data['all_companies'] = $this->Company_model->get_company();
	...
	public function jobs_applied()
     {
		...
		$data['title'] = $this->lang->line('left_jobs_applied').' | '.$this->Core_model->site_title();
    ...
    public function jobs_applied_list()
     {
		...
		// get user
		$user = $this->Core_model->read_user_info($r->user_id);
		// get full name
		if(!is_null($user)){
			$full_name = $user[0]->first_name. ' ' .$user[0]->last_name;
			$uemail = $user[0]->email;
	...
	public function jobs_interviews()
     {
		...
		$data['title'] = $this->lang->line('left_job_interviews').' | '.$this->Core_model->site_title();
		$data['all_employees'] = $this->Core_model->all_employees();
    ...
    public function jobs_interview_list()
     {
		...
		// get date
		$interview_date = $this->Core_model->set_date_format($r->interview_date);		
		...
		foreach(explode(',',$r->interviewers_id) as $interviewers_id) {
			$user_intwer = $this->Core_model->read_user_info($interviewers_id);
		...
		// interview added by
		$u_added = $this->Core_model->read_user_info($r->added_by);
		// user full name
		if(!is_null($u_added)){
	...
	public function announcement()
     {
		...
		$data['title'] = $this->lang->line('xin_announcements').' | '.$this->Core_model->site_title();
    ...
    public function announcement_list()
     {
		...
		// get user > added by
		$user = $this->Core_model->read_user_info($r->published_by);
		// user full name
		if(!is_null($user)){
			$full_name = $user[0]->first_name.' '.$user[0]->last_name;
		} else {
	...
	public function expense_claims()
     {
		...
		$data['title'] = $this->lang->line('xin_expenses').' | '.$this->Core_model->site_title();
    ...
    public function expense_list()
     {
		...
		$data = array();
          ...
          foreach($expense->result() as $r) {
			...
			// get date
			$edate = $this->Core_model->set_date_format($r->purchase_date);
			// get currency
			$currency = $this->Core_model->currency_sign($r->amount);
			// download
			$download = '';
			// get company
			$company = $this->Core_model->read_company_info($r->company_id);
	...
	public function travel()
     {
		...
		$data['title'] = $this->lang->line('left_travels').' | '.$this->Core_model->site_title();
		$data['all_employees'] = $this->Core_model->all_employees();
	...
	public function payslip()
     {
		...
		$data['title'] = $this->lang->line('xin_payslip').' | '.$this->Core_model->site_title();
		$data['all_employees'] = $this->Core_model->all_employees();
	 ...
	 public function employee_payslip_list()
     {
		...
		$data = array();
          ...
          foreach($history->result() as $r) {
			  ...
			  $p_amount = $this->Core_model->currency_sign($r->payment_amount);
	 ...
	 public function upayslip()
     {
		...
		$data['all_employees'] = $this->Core_model->all_employees();
		$data = array(
				'title' => $this->lang->line('xin_payroll_employee_payslip').' | '.$this->Core_model->site_title(),
	...
	public function update_travel() {
	 ...
	 public function advance_salary()
     {
		...
		$data['title'] = $this->lang->line('xin_advance_salary').' | '.$this->Core_model->site_title();
		$data['all_employees'] = $this->Core_model->all_employees();
		$data['all_companies'] = $this->Company_model->get_company();
	 ...
	 public function advance_salary_report()
     {
		...
		$data['title'] = $this->lang->line('xin_advance_salary_report').' | '.$this->Core_model->site_title();
		$data['all_employees'] = $this->Core_model->all_employees();
		$data['all_companies'] = $this->Company_model->get_company();
    ...
    public function advance_salary_list()
     {
		...
		$data = array();
          ...
          foreach($advance_salary->result() as $r) {
			...
			// get net salary
			$advance_amount = $this->Core_model->currency_sign($r->advance_amount);
			// get date > created at > and format
			$cdate = $this->Core_model->set_date_format($r->created_at);
			// get status
			...
			$monthly_installment = $this->Core_model->currency_sign($r->monthly_installment);
    ...
    public function advance_salary_report_list()
     {
		...
		$data = array();
          ...
          foreach($advance_salary->result() as $r) {
			...
			$ramount = $this->Core_model->currency_sign($remainig_amount);
			...
			//total paid
			$total_paid = $this->Core_model->currency_sign($r->total_paid);
			// get company
			$company = $this->Core_model->read_company_info($r->company_id);
			if(!is_null($company)){
				$comp_name = $company[0]->name;
