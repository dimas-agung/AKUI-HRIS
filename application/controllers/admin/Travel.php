...
class Travel extends MY_Controller {
	...
	// =================================================================================================================
		...
		public function index()
	    {
			...
			$data['transport_arrangement_types'] = $this->Travel_model->transport_arrangement_types();
			$data['get_all_companies']        = $this->Company_model->get_company();
			
			$role_resources_ids = $this->Core_model->user_role_resource();
			if(in_array('0641',$role_resources_ids)) {
				if(!empty($session)){ 
	    ...
	    public function travel_list()
	    {
			...
			$cek_transport = $this->Core_model->read_transport_info($r->travel_mode);
			if(!is_null($cek_transport)){
				$transport = $cek_transport[0]->type;
			} else {
				$transport = '--';	
			}
	

			if(in_array('0643',$role_resources_ids)) { //edit
				$edit = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_edit').'">
							<button type="button" class="btn icon-btn btn-xs btn-default waves-effect waves-light"  data-toggle="modal" data-target=".edit-modal-data"  data-travel_id="'. $r->travel_id . '"><span class="fa fa-check-circle"></span> Proses </button></span> ';
			...
			if(in_array('0644',$role_resources_ids)) { // delete
				$delete = '<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'">
								<button type="button" class="btn icon-btn btn-xs btn-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->travel_id . '">
			...
			if(in_array('0645',$role_resources_ids)) { //view
			...
			$iemployee_name_info  = $r->visit_purpose;
	...
	// =================================================================================================================
	    ...
	    public function read()
		{
			...
			$data = array(
					...
					'transport_arrangement_types' => $this->Travel_model->transport_arrangement_types(),
				);
			$session = $this->session->userdata('username');
			if(!empty($session)){ 
