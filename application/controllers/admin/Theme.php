...
class Theme extends MY_Controller
{
   ...
   /*Function to set JSON output*/
	...
	public function index() {
		...
		$data['title'] = $this->lang->line('xin_theme_settings').' | '.$this->Core_model->site_title();
		$setting = $this->Core_model->read_setting_info(1);
		$company_info = $this->Core_model->read_company_setting_info(1);
		$theme_info = $this->Core_model->read_theme_info(1);
		$data = array(
			'title' => $this->lang->line('xin_theme_settings').' | '.$this->Core_model->site_title(),
	...
	public function page_layouts() {
		...
		if($this->input->post('page_header')==='') {
        	...
        	$Return['error'] = $this->lang->line('xin_hris_themes_options_error_field');
		} else if($this->input->post('dashboard_option')==='') {
        	$Return['error'] = $this->lang->line('xin_hris_dashboard_options_error_field');
		} else if($this->input->post('login_page_options')==='') {
        	$Return['error'] = $this->lang->line('xin_hris_login_page_options_error_field');
		}
		...
		$result = $this->Core_model->update_theme_info_record($data,$id);		
	...
	public function logo_info() {
		...
		$result = $this->Core_model->update_company_info_record($data,$id);	
		if ($result == TRUE) {
