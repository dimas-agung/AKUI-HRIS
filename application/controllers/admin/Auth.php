...
class Auth extends MY_Controller
{
	...
	public function login() {
		...
		$this->form_validation->set_rules('inik', 'NIK', 'trim|required|xss_clean');
		...
		if($username==='') {
			...
			$Return['error'] = $this->lang->line('xin_employee_error_login_nip');
		
		} elseif($password===''){
		
			$Return['error'] = $this->lang->line('xin_employee_error_login_password');
		...
		// if ($result == TRUE) {
				...
				$session_data = array(
					...
					'username'   => $result[0]->employee_id,
					'company'    => $result[0]->company_id,
					'companies'  => $result[0]->view_company_id,
					'nip' 	     => $result[0]->employee_id,
					'email'      => $result[0]->email,
				);
				// Add user data in session
				$this->session->set_userdata('nip',       $session_data);
				$this->session->set_userdata('username',  $session_data);
				$this->session->set_userdata('company',   $session_data);
				$this->session->set_userdata('companies', $session_data);
				...
				$this->Core_model->login_update_record($last_data, $id);

				// ----------------------------------------------------------------
				// log activity
				// ----------------------------------------------------------------
					// $this->Core_model->add_log_activity($modul_name,$fitur_name,$isi,$proses,$status); - default
					// $this->Core_model->add_log_activity('Login','Login','Masuk Aplikasi HRIS','Log in','Sukses');
				// ----------------------------------------------------------------
	...
	public function unlock() {
		...
		$username = $iresult[0]->employee_id;
		$data = array(
			'username' => $username,
			'password' => $password
			);
	...
	public function send_mail() {
		...
		if($this->input->post('iemail')) {
			...
			//get email template
			$template = $this->Core_model->read_email_template(2);
			//get employee info
			$query = $this->Core_model->read_user_info_byemail($this->input->post('iemail'));
			...
			if($user > 0) {
				...
				hris_mail($cinfo[0]->email,$cinfo[0]->company_name,$this->input->post('iemail'),$subject,$body);			
	...
	public function reset_password() {
		...
		if($this->input->get('change') == 'true'){
			...
			if($this->input->get('email')) {
				...
				$cinfo = $this->Core_model->read_company_setting_info(1);
				//get email template
				$template = $this->Core_model->read_email_template(17);
				//get employee info
				$query = $this->Core_model->read_user_info_byemail($this->input->get('email'));
				...
				if($user > 0) {
					...
					hris_mail($cinfo[0]->email,$cinfo[0]->company_name,$this->input->get('email'),$subject,$body);				
