...
class Quotes_model extends CI_Model {
	...
	{
	  return $this->db->get("xin_hris_quotes");
	}
	...
	public function read_quote_converted_info($id) {
		...
		$sql = 'SELECT * FROM xin_hris_quotes WHERE quote_id = ? and status = ?';
		$binds = array($id,1);
	...
	public function read_invoice_items_info($id) {
		...
		$this->db->select('*');
		$this->db->from('xin_hris_quotes_items');
		$this->db->where($condition);
		$this->db->limit(1);
		$query = $this->db->get();
	...
	public function get_quote_items($id) {
		...
		$sql = 'SELECT * FROM xin_hris_quotes_items WHERE quote_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds); 
	...
	public function add_quote_record($data){
		$this->db->insert('xin_hris_quotes', $data);
	...
	public function add_quote_items_record($data){
		$this->db->insert('xin_hris_quotes_items', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
	...
	public function delete_record($id){
		...
		$this->db->delete('xin_hris_quotes');
		
	}
	// Function to Delete selected record from table
	public function delete_quote_items($id){
		$this->db->where('quote_id', $id);
		$this->db->delete('xin_hris_quotes_items');
	...
	public function update_quote_record($data, $id){
		$this->db->where('quote_id', $id);
		if( $this->db->update('xin_hris_quotes',$data)) {
			return true;
		} else {
			return false;
		}		
	...
	public function update_quote_items_record($data, $id){
		$this->db->where('quote_item_id', $id);
		if( $this->db->update('xin_hris_quotes_items',$data)) {
			return true;
		} else {
			return false;
		}		
