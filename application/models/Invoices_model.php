...
class Invoices_model extends CI_Model {
	...
	{
	  return $this->db->get("xin_hris_invoices");
	}
	...
	public function get_employee_project_invoices($id) {
		...
		$sql = 'SELECT * FROM xin_hris_invoices WHERE project_id = ?';
		$binds = array($id);
	...
	public function get_completed_invoices() {
		...
		$sql = 'SELECT * FROM xin_hris_invoices WHERE status = ?';
		$binds = array(1);
		$query = $this->db->query($sql, $binds);
	 	return $query->result();
	}
	...
	public function read_invoice_info($id) {
		...
		$this->db->select('*');
		$this->db->from('xin_hris_invoices');
	...
	public function read_invoice_items_info($id) {
		...
		$this->db->select('*');
		$this->db->from('xin_hris_invoices_items');
		$this->db->where($condition);
		$this->db->limit(1);
		$query = $this->db->get();
	...
	public function get_invoice_items($id) {
		...
		$sql = 'SELECT * FROM xin_hris_invoices_items WHERE invoice_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds); 
	...
	public function get_client_invoices($id) {
		...
		$sql = 'SELECT * FROM xin_hris_invoices WHERE client_id = ?';
		$binds = array($id);
		$query = $this->db->query($sql, $binds); 
  	     return $query;
	}
	...
	{
	     $sql = 'SELECT * FROM xin_hris_invoices where client_id = ? order by invoice_id desc limit ?';
		 $binds = array($id,5);
		 $query = $this->db->query($sql, $binds); 
	...
	public function add_invoice_record($data){
		$this->db->insert('xin_hris_invoices', $data);
	...
	public function add_invoice_items_record($data){
		$this->db->insert('xin_hris_invoices_items', $data);
		if ($this->db->affected_rows() > 0) {
			return true;
		} else {
			return false;
	...
	public function delete_record($id){
		...
		$this->db->delete('xin_hris_invoices');
	...
	public function delete_invoice_items($id){
		$this->db->where('invoice_id', $id);
		$this->db->delete('xin_hris_invoices_items');
	...
	public function update_invoice_record($data, $id){
		$this->db->where('invoice_id', $id);
		if( $this->db->update('xin_hris_invoices',$data)) {
			return true;
		} else {
			return false;
		}		
	...
	public function update_invoice_items_record($data, $id){
		$this->db->where('invoice_item_id', $id);
		if( $this->db->update('xin_hris_invoices_items',$data)) {
			return true;
		} else {
			return false;
		}		
