...
if(isset($_GET['jd']) && isset($_GET['sick_id']) && $_GET['data']=='sick'){
?>
<?php $session = $this->session->userdata('username');?>
<?php $user_info = $this->Core_model->read_user_info($session['user_id']);?>
<div class="modal-header">
  <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">×</span> </button>
  <h4 class="modal-title" id="edit-modal-data"><?php echo $this->lang->line('xin_edit_sick');?></h4>
</div>
<?php $attributes = array('name' => 'edit_sick', 'id' => 'edit_sick', 'autocomplete' => 'off', 'class'=>'m-b-1');?>
<?php $hidden = array('_method' => 'EDIT', '_token' => $sick_id, 'ext_name' => $sick_id);?>
<?php echo form_open('admin/permission/edit_sick/'.$sick_id, $attributes, $hidden);?>
<?php $session = $this->session->userdata('username');?>
<?php $user = $this->Core_model->read_employee_info($session['user_id']);?>
  ...
  <div class="modal-body">
    ...
    <div class="col-md-12">
        <div class="form-group">
          <label for="reason"><?php echo $this->lang->line('xin_sick_reason');?></label>
          <textarea class="form-control" placeholder="<?php echo $this->lang->line('xin_sick_reason');?>" name="reason" cols="30" rows="3" id="reason"><?php echo $reason;?></textarea>
        </div>
    </div>
    </div>
  </div>
 ...
 $(document).ready(function(){
	...
	e.preventDefault();
		...
		$.ajax({
			type: "POST",
			url: e.target.action,
			data: obj.serialize()+"&is_ajax=2&edit_type=sick&form="+action,
			cache: false,
			success: function (JSON) {
				if (JSON.error != '') {
					...
					var xin_table = $('#xin_table').dataTable({
						"bDestroy": true,
						"ajax": {
							url : "<?php echo site_url("admin/permission/sick_list") ?>",
							type : 'GET'
						},
						// dom: 'lBfrtip',
            "columns": [
...
<?php } else if(isset($_GET['jd']) && isset($_GET['sick_id']) && $_GET['data']=='view_sick'){
?>
<div class="modal-header">
  <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">×</span> </button>
  <h4 class="modal-title" id="edit-modal-data"><?php echo $this->lang->line('xin_view');?> <?php echo $this->lang->line('left_sick');?></h4>
</div>
<form class="m-b-1">
  <div class="modal-body">
    <table class="footable-details table table-striped table-hover toggle-circle">
      <tbody>
        ...
        <tr>
          <th><?php echo $this->lang->line('xin_sick_type');?></th>
          <td style="display: table-cell;"><?php foreach($all_sick_types as $type) {?>
            <?php if($type->sick_type_id==$sick_type_id):?> <?php echo $type->type_name;?> <?php endif;?>
            <?php } ?></td>
        </tr>
        <tr>
        ...
        <tr>
          <th><?php echo $this->lang->line('xin_sick_reason');?></th>
