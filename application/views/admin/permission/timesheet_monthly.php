...
if($user_info[0]->user_role_id==1){
	...
	if(!isset($month_year)){
		...
		$month = date('m', $date);
		$year = date('Y', $date);
		$xin_employees = $this->Permission_model->get_xin_employees();
	} else {
		...
		if($this->input->post('employee_id')==0){
			...
			$xin_employees = $this->Core_model->read_user_info($this->input->post('employee_id'));
		}
	}
	...
	$xin_employees = $this->Core_model->read_user_info($session['user_id']);
}
// total days in month
  ...
  <div class="box-body">
    <div class="row">
      <div class="col-md-12">
        ...
        <?php echo form_open('admin/permission/', $attributes, $hidden);?>
        <div class="row">
          <div class="col-md-2">
          ...
          <div class="col-md-3">
            <div class="form-group">
              ...
              <br />
              <?php echo form_button(array('name' => 'hris_form', 'type' => 'submit', 'class' => 'btn btn-primary', 'content' => '<i class="fa fa fa-check-square-o"></i> '.$this->lang->line('xin_get'))); ?> </div>
          </div>
        </div>
      ...
      <table class="datatables-demo table table-striped table-bordered" id="xin_table">
        ...
        <tbody>
          ...
          <tr>
			...
			$user_id = $r->user_id;
			// office shift
			$office_shift = $this->Permission_model->read_office_shift_information($r->office_shift_id);
			// get holiday
			$h_date_chck = $this->Permission_model->holiday_date_check($attendance_date);
			$holiday_arr = array();
			if($h_date_chck->num_rows() == 1){
				$h_date = $this->Permission_model->holiday_date($attendance_date);
			...
			// get leave/employee
			$leave_date_chck = $this->Permission_model->leave_date_check($r->user_id,$attendance_date);
			$leave_arr = array();
			if($leave_date_chck->num_rows() == 1){
				$leave_date = $this->Permission_model->leave_date($r->user_id,$attendance_date);
			...
			$attendance_status = '';
			$check = $this->Permission_model->attendance_first_in_check($r->user_id,$attendance_date);
			...
			$attendance = $this->Permission_model->attendance_first_in($r->user_id,$attendance_date);
