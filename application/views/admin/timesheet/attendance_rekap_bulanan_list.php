...
if (!isset($month_year)) {
  $xin_bulan     = $this->Timesheet_model->get_xin_employees_bulan(date('Y-m'));
  if ($xin_bulan) {
...
<?php $attributes = array('name' => 'attendance_rekap_bulanan_proses', 'id' => 'attendance_rekap_bulanan_proses', 'autocomplete' => 'off', 'class' => 'add form-hrm'); ?>
<?php $hidden     = array('user_id' => $session['user_id']); ?>
<?php echo form_open('admin/timesheet/attendance_rekap_bulanan_proses', $attributes, $hidden); ?>
<?php
$data = array(
  'type'     => 'hidden',
  'name'     => 'date_format',
  ...
  <div class="col-md-12">
    <div class="box mb-4">
      <div class="box-header with-border">
        <h3 class="box-title"> Proses Rekap Absensi Bulanan </h3>
      </div>
      <div class="box-body">
        <div class="row">
          ...
          <div class="col-md-2">
            <div class="form-group">
              ...
              <select class="form-control input-sm" name="month_year" id="month_year" data-plugin="select_hrm" data-placeholder="<?php echo $this->lang->line('xin_e_details_month'); ?>" required>
                ...
                <?php
                  foreach ($all_bulan_gaji as $bulan_gaji) {
                ...
                ?>
                  <option
                    value="<?php echo $bulan_gaji->month_payroll; ?>"
                    data-title='<?= json_encode($title_dates) ?>'
                    <?php if ($bulan_gaji->month_payroll == $bulan) : ?> selected="selected" <?php endif; ?>
                  >
  ...
  <div class="box-header ">
    ...
    <div class="box-tools pull-right" id="myBtn" style="display:none;">
      ...
      <?php if (in_array('0932', $role_resources_ids)) { ?>
        <button type="submit" class="btn  btn-warning save" title="Rekap Absensi Bulanan">
          <i class="fa fa-gears"></i> Rekap Absensi Bulanan
        </button>
      <?php } ?>
...
<script type="text/javascript">
  ...
  function searchDataAttendance() {
    ...
    const titles = jQuery('#month_year :selected').data('title');

    if (titles) {
      jQuery('.row_dates th').each(function(index) {
        $(this).find('center').text(titles[index]);
      });
    }
    ...
    // }
      ...
      $(".info_rekap").html('<h3 class="box-title text-uppercase text-bold"> Rekap Absensi Bulanan, Periode : <?php echo $start_att; ?> sampai <?php echo $end_att; ?></h3><h5><i class="fa fa-warning"></i> Silahkan Klik Tombol "<span class="blink blink-two kuning">Rekap Absensi Bulanan </span>" Jika Data Ditabel ini tidak ada.</h5>');
      $('#p_month').html($bulan);

      toastr.success('Tampilkan Rekap Absensi Bulanan');
      ...
      var xin_table3 = $('#xin_table_recap').dataTable({
        ...
        "ajax": {
          url: site_url + "timesheet/attendance_rekap_bulanan_list/?company_id=" + company_id + "&month_year=" + month_year + "&pola_kerja=" + pola_kerja,
          type: 'GET'
        },
        ...
        "rowCallback": function(row, data, index) {
          ...
          if (data[11] == '') {
            ...
            $(row).find('td:eq(10)').css('background-color', '#ddd');
            $(row).find('td:eq(10)').css('color', 'black');

          } else if (data[12] == '') {

            $(row).find('td:eq(11)').css('background-color', '#ddd');
