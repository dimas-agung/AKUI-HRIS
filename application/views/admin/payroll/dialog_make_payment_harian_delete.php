...
if(isset($_GET['jd']) && isset($_GET['payslip_id']) && $_GET['data']=='payment' && $_GET['type']=='dayly_payment_delete'){ 
...
?>
		...
		<div class="modal-header">
		  ...
		  <h4 class="modal-title" id="edit-modal-data"> <i class="fa fa-trash"></i> 
		  		Hapus Gaji Harian - Per Karyawan!
			</h4>
		</div>

		<div class="modal-body" style="overflow:auto; height:670px;">
			
			<?php $attributes = array('name' => 'del_pay_dayly', 'id' => 'del_pay_dayly', 'autocomplete' => 'off', 'class'=>'m-b-1');?>
			<?php $hidden     = array('_method' => 'ADD');?>
			<?php echo form_open('admin/payroll/del_pay_dayly/', $attributes, $hidden);?>
			...
			<input type="hidden" value="<?php echo $total_gaji;?>"                            name="total_gaji"                id="total_gaji"/>
			<input type="hidden" value="<?php echo $jumlah_overtime;?>"                       name="jumlah_overtime"           id="jumlah_overtime"/>
			...
			<input type="hidden" value="<?php echo $employee_email;?>"                        name="eemployee_email"           id="eemployee_email"/>			
	...
	$(document).ready(function(){
		...
		$("#del_pay_dayly").submit(function(e){
			...
			$.ajax({
				type: "POST",
				url: e.target.action,
				data: obj.serialize()+"&is_ajax=11&data=dayly&proses_type=del_dayly_payment&form="+action,
				cache: false,
				success: function (JSON) {
					...
					if (JSON.error != '') {
						...
						var xin_table3   = $('#xin_table_harian').dataTable({
							...
							"ajax": {
								url : "<?php echo site_url("admin/payroll/payslip_list_harian") ?>?company_id=<?php echo $company_id;?>&start_date=<?php echo $this->input->get('start_date');?>&end_date=<?php echo $this->input->get('end_date');?>",
								type : 'GET'
							},
							  "columns": [
