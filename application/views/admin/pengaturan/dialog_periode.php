...
<?php if(isset($_GET['jd']) && isset($_GET['payroll_id']) && $_GET['data']=='view_periode'){ ?>

    <div class="modal-header">
      <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">×</span> </button>
      <h4 class="modal-title" id="edit-modal-data"> <i class="fa fa-eye"></i> Lihat Periode</h4>
    </div>
    <form class="m-b-1">
    <div class="modal-body">
      <table class="footable-details table table-striped table-hover toggle-circle">
        <tbody>
          ...
          <tr>
            <th> Jenis </th>
            <td style="display: table-cell;">: <?php if($jenis=='1'): $status = 'Harian';?>
              <?php endif; ?>
              <?php if($jenis=='2'): $status = 'Borongan';?>
              <?php endif; ?>
              <?php echo $jenis;?></td>
          </tr>
...
<?php } else if(isset($_GET['jd']) && isset($_GET['payroll_id']) && $_GET['data']=='periode'){ ?>
   
    <?php $session = $this->session->userdata('username');?>
    <?php $user_info = $this->Core_model->read_user_info($session['user_id']);?>
    <div class="modal-header">
      <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">×</span> </button>
      <h4 class="modal-title" id="edit-modal-data"> <i class="fa fa-pencil"></i> Edit Periode</h4>
    </div>
    <?php $attributes = array('name' => 'edit_periode', 'id' => 'edit_periode', 'autocomplete' => 'off', 'class'=>'m-b-1');?>
    <?php $hidden = array('_method' => 'EDIT', '_token' => $payroll_id, 'ext_name' => $payroll_id);?>
    <?php echo form_open('admin/pengaturan/edit_periode/'.$payroll_id, $attributes, $hidden);?>
    <div class="modal-body">
      <div class="row">
        <div class="col-md-6">
            ...
            <div class="col-md-6">
             ...
             <div class="col-md-6">
               <div class="form-group">
                ...
                <select name="jenis" class="select2" data-plugin="select_hrm" data-placeholder="Jenis">
                  <option value="2" <?php if($jenis=='2') { ?> selected <?php } ?>>  Harian </option>
                  <option value="3" <?php if($jenis=='3') { ?> selected <?php } ?>>  Borongan </option>
                </select>
              </div>
            </div>
          </div>
    ...
    <script type="text/javascript">
      $(document).ready(function(){
          ...
          /* Edit*/
        	$("#edit_periode").submit(function(e){
          	  /*Form Submit*/
          	  e.preventDefault();
          		...
          		$.ajax({
          			type: "POST",
          			url: e.target.action,
          			data: obj.serialize()+"&is_ajax=2&edit_type=periode&form="+action,
          			cache: false,
          			success: function (JSON) {
          				if (JSON.error != '') {
          					...
          					var xin_table = $('#xin_table').dataTable({
          						"bDestroy": true,
          						"ajax": {
          							url : "<?php echo site_url("admin/pengaturan/periode_list") ?>",
          							type : 'GET'
          						},
          						"columns": [
