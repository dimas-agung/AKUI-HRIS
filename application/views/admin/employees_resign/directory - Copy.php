...
<?php if($user_info[0]->user_role_id==1 || $user_info[0]->user_role_id==4 ||$user_info[0]->user_role_id==5 || $user_info[0]->user_role_id==6){ ?>
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="row">
            ...
            <div class="col-md-1"><label for="designation">&nbsp;</label><?php echo form_button(array('name' => 'hris_form', 'type' => 'submit', 'class' => $this->Core_model->form_button_class(), 'content' => '<i class="fa fa fa-check-square-o"></i> '.$this->lang->line('xin_get'))); ?>
            </div>
        </div>
...
<?php if($total_record > 0) {?>
  ...
  <?php
	...
	?>
  <?php $company = $this->Company_model->read_company_information($employee->company_id);?>
  <?php
    if(!is_null($company)){
    $company_name = strtoupper($company[0]->name);
    } else {
    $company_name = '--'; 
    }
  ?>
  ...
  <?php
    ...
    $department_name = strtoupper($department[0]->department_name);
    } else {
    $department_name = '--'; 
    }
  ?>
  ...
  <div class="col-6 col-lg-3">
    <div class="box box-body">
      <div class="text-center"> <a href="#"> <img class="hr-user-img user-img-xin rounded-circle xin-mb-5" src="<?php echo $u_file;?>" alt="<?php echo $employee->first_name;?> <?php echo $employee->last_name;?>"> </a>
        ...
        <h6 class="user-info mt-0 xin-mb-5 text-lighter"><?php echo ucwords($company_name);?></h6>
        <h6 class="user-info mt-0 xin-mb-5 text-lighter"><?php echo ucwords($department_name);?></h6>
        ...
        <!-- <p><?php echo $designation_name;?> </p> -->

        <?php if(in_array('202',$role_resources_ids)) {?>
        <a href="<?php echo site_url('admin/employees/detail')?>/<?php echo $employee->user_id;?>" class="btn btn-rounded btn-primary btn-sm"><?php echo $this->lang->line('xin_detail_profil');?> 
            <i class="fa fa-arrow-circle-right"></i>
        </a> 
        <a href="<?php echo site_url('admin/employees/download_profile')?>/<?php echo $employee->user_id;?>" class="btn btn-rounded btn-primary btn-sm" target="_blank"><?php echo $this->lang->line('xin_download_profil');?> <i class="fa fa-download"></i></a>
        <?php } else {?>
        <?php } ?>
      </div>
    </div>
