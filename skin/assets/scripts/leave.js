$(document).ready(function() {
   var xin_table = $('#xin_table').dataTable({
		...
		"ajax": {
            url : site_url+"permission/leave_list/",
            type : 'GET'
        },
        "columns": [
	...
	// 	jQuery.get(base_url+"/get_remain_employees/"+jQuery(this).val(), function(data, status){
	// 		jQuery('#employee_ajax').html(data);
	// 	});
	// });
	...
	$("#ihr_report").submit(function(e){
		...
		e.preventDefault();
			 var xin_table2 = $('#xin_table').dataTable({
				"bDestroy": true,
				"ajax": {
					url : site_url+"permission/leave_list/?ihr=true&company_id="+$('#aj_companyf').val()+"&employee_id="+$('#employee_id').val()+"&status="+$('#status').val(),
					type : 'GET'
				},
				"fnDrawCallback": function(settings){
					$('[data-toggle="tooltip"]').tooltip();          
		...
		$.ajax({
			...
			success: function (JSON) {
				if (JSON.error != '') {
					...
					toastr.success('Proses Hapus Data Berlangsung');
	...
	$('.edit-modal-data').on('show.bs.modal', function (event) {
		...
		var modal = $(this);
	$.ajax({
		url : site_url+"permission/read_leave_record/",
		type: "GET",
...
$( document ).on( "click", ".delete", function() {
	...
	$('#delete_record').attr('action',site_url+'permission/delete_leave/'+$(this).data('record-id'));
});