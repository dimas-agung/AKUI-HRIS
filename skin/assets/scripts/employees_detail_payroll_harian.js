$(document).ready(function(){			
   ...
   // Month & Year
	...
	// =====================================================================================================
	    ...
	    // Lembur
	   	var xin_table_emp_overtime = $('#xin_table_all_overtime_harian').dataTable({
	        "bDestroy": true,
			"ajax": {
	            url : site_url+"payroll/salary_all_overtime_harian/"+$('#user_id').val()+"/"+$('#start_date').val()+"/"+$('#end_date').val(),
	            type : 'GET'
	        },
	        "columns": [
	  	...
	  	// Gapok
		var xin_table_gapok_ad = $('#xin_table_all_gapok_harian').dataTable({
	        "bDestroy": true,
			"ajax": {
	            url : site_url+"payroll/salary_all_gapok_harian/"+$('#user_id').val(),
	            type : 'GET'
	        },
	        "columns": [
	    ...
	    // Insentif
		var xin_table_commissions_ad = $('#xin_table_all_commissions_harian').dataTable({
	        "bDestroy": true,
			"ajax": {
	            url : site_url+"payroll/salary_all_commissions_harian/"+$('#user_id').val(),
	            type : 'GET'
	        },
	        "columns": [
				{"name": "kolom_1","orderable": false,"searchable": false,  "className": "text-center", "width": "8%"},
	    ...
	    // BPJS
		var xin_table_statutory_deductions_ad = $('#xin_table_all_statutory_deductions_harian').dataTable({
	        "bDestroy": true,
			"ajax": {
	            url : site_url+"payroll/salary_all_statutory_deductions_harian/"+$('#user_id').val(),
	            type : 'GET'
	        },
	        "columns": [
				{"name": "kolom_1","orderable": false,"searchable": false,  "className": "text-center", "width": "8%"},
	    ...
	    // Pemotongan
		var xin_table_minus_ad = $('#xin_table_all_minus_harian').dataTable({
	        "bDestroy": true,
			"ajax": {
	            url : site_url+"payroll/salary_all_minus_harian/"+$('#user_id').val(),
	            type : 'GET'
	        },
	         "columns": [
					{"name": "kolom_1","orderable": false,"searchable": false,  "className": "text-center", "width": "8%"},
	...
	// ======================================================================================================
		...
		jQuery("#employee_update_gapok").submit(function(e){
			...
			jQuery.ajax({
				type: "POST",
				url: e.target.action,
				data: obj.serialize()+"&is_ajax=4&data=employee_update_gapok&type=employee_update_gapok&form="+action,
				cache: false,
				success: function (JSON) {
					if (JSON.error != '') {
						...
						xin_table_gapok_ad.api().ajax.reload(function(){ 
							
							alert_success('Sukses',JSON.result);
							$('input[name="csrf_hris"]').val(JSON.csrf_hash);
						}, true);
						
						jQuery('#employee_update_gapok')[0].reset(); // To reset form fields
						jQuery('.save').prop('disabled', false);
					}
				}
			});
		...
		jQuery("#statutory_deductions_info_harian").submit(function(e){
			...
			jQuery.ajax({
				type: "POST",
				url: e.target.action,
				data: obj.serialize()+"&is_ajax=4&data=statutory_deductions_info_harian&type=statutory_deductions_info_harian&form="+action,
				cache: false,
				success: function (JSON) {
					if (JSON.error != '') {
						...
						jQuery('#statutory_deductions_info_harian')[0].reset(); // To reset form fields
						jQuery('.save').prop('disabled', false);
					}
				}
			});
	...
	{		
		...
		if(field_tpe == 'salary_commissions_harian'){
			var field_add = '&data=salary_commissions_info&type=salary_commissions_info&';

		} else if(field_tpe == 'salary_gapok_harian'){
			var field_add = '&data=salary_gapok_info&type=salary_gapok_info&';

		} else if(field_tpe == 'salary_minus_harian'){
			var field_add = '&data=salary_minus_info&type=salary_minus_info&';

		} else if(field_tpe == 'salary_statutory_deductions_harian'){
			var field_add = '&data=salary_statutory_deductions_info_harian&type=salary_statutory_deductions_info_harian&';
   ...
   /* Delete data */
	$("#delete_record").submit(function(e){
		...
		if(tk_type == 'all_overtime'){
			var field_add = '&is_ajax=30&data=delete_record&type=delete_all_overtime&';
			var tb_name = 'xin_table_'+tk_type;
		
		} else if(tk_type == 'all_commissions_harian'){
			var field_add = '&is_ajax=30&data=delete_record&type=delete_all_commissions_harian&';
			var tb_name = 'xin_table_'+tk_type;

		} else if(tk_type == 'all_gapok_harian'){
			var field_add = '&is_ajax=30&data=delete_record&type=delete_all_gapok_harian&';
			var tb_name = 'xin_table_'+tk_type;

		} else if(tk_type == 'all_statutory_deductions_harian'){
			var field_add = '&is_ajax=30&data=delete_record&type=delete_all_statutory_deductions_harian&';
			var tb_name = 'xin_table_'+tk_type;

		} else if(tk_type == 'all_minus_harian'){
			var field_add = '&is_ajax=30&data=delete_record&type=delete_all_minus_harian&';
			var tb_name = 'xin_table_'+tk_type;	
