$(document).ready(function() {
   var xin_table = $('#xin_table').dataTable({
	...
	$('.edit-modal-data').on('show.bs.modal', function (event) {
		...
		var pinjam_id = button.data('field_id');
		var modal = $(this);
		$.ajax({
			url : base_url+"/asset_pinjam_read/",
			type: "GET",
			data: 'jd=1&is_ajax=1&mode=modal&data=eassets_pinjam&pinjam_id='+pinjam_id,
	...
	$('.view-modal-data').on('show.bs.modal', function (event) {
		...
		$.ajax({
			url :  base_url+"/asset_pinjam_read/",
			type: "GET",
			data: 'jd=1&is_ajax=1&mode=modal&data=view_asset_pinjam&type=view_asset_pinjam&pinjam_id='+pinjam_id,
			success: function (response) {
				if(response) {
	...
	$("#xin-form").submit(function(e){
		...
		fd.append("is_ajax", 1);
		fd.append("add_type", 'add_asset_pinjam');
...
$( document ).on( "click", ".delete", function() {
	...
	$('#delete_record').attr('action',base_url+'/delete_asset_pinjam/'+$(this).data('record-id'));
});