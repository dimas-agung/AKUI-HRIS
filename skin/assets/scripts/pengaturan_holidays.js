...
var xin_table = $('#xin_table').dataTable({
	"bDestroy": true,
	"ajax": {
		url : site_url+"pengaturan/holidays_list/",
		type : 'GET'
	},
...
$("#ihr_report").submit(function(e){
	...
	e.preventDefault();
		 var xin_table2 = $('#xin_table').dataTable({
			"bDestroy": true,
			"ajax": {
				url : site_url+"pengaturan/holidays_list/?ihr=true&company_id="+$('#aj_companyf').val()+"&status="+$('#status').val(),
				type : 'GET'
			},
			"fnDrawCallback": function(settings){
				$('[data-toggle="tooltip"]').tooltip();          
...
$('.edit-modal-data').on('show.bs.modal', function (event) {
	...
	var modal = $(this);
	$.ajax({
		url : site_url+"pengaturan/read_holiday_record/",
		type: "GET",
...
$( document ).on( "click", ".delete", function() {
	...
	$('#delete_record').attr('action',site_url+'pengaturan/delete_holiday/'+$(this).data('record-id'));
});