$(document).ready(function() {
   var xin_table = $('#xin_table').dataTable({
		...
		"ajax": {
            url : site_url+"permission/sick_list/",
            type : 'GET'
        },
        "columns": [
	...
	jQuery("#aj_company").change(function(){
		jQuery.get(base_url+"/get_sick_employees/"+jQuery(this).val(), function(data, status){
	...
	$("#ihr_report").submit(function(e){
		...
		e.preventDefault();
			 var xin_table2 = $('#xin_table').dataTable({
				"bDestroy": true,
				"ajax": {
					url : site_url+"permission/sick_list/?ihr=true&company_id="+$('#aj_companyf').val()+"&employee_id="+$('#employee_id').val()+"&status="+$('#status').val(),
					type : 'GET'
				},
				"fnDrawCallback": function(settings){
					$('[data-toggle="tooltip"]').tooltip();          
	...
	$('.edit-modal-data').on('show.bs.modal', function (event) {
		...
		var sick_id = button.data('sick_id');
		var modal = $(this);
	$.ajax({
		url : site_url+"permission/read_sick_record/",
		type: "GET",
		data: 'jd=1&is_ajax=1&mode=modal&data=sick&sick_id='+sick_id,
...
$( document ).on( "click", ".delete", function() {
	...
	$('#delete_record').attr('action',site_url+'permission/delete_sick/'+$(this).data('record-id'));
});