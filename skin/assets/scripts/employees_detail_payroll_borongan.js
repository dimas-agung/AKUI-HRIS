$(document).ready(function(){			
	...
	// =====================================================================================================
		...
		var xin_table_bank_account = $('#xin_table_bank_account').dataTable({
	  	...
	  	// Insentif
		var xin_table_commissions_ad = $('#xin_table_all_commissions_borongan').dataTable({
	        "bDestroy": true,
			"ajax": {
	            url : site_url+"payroll/salary_all_commissions_borongan/"+$('#user_id').val(),
	            type : 'GET'
	        },
	        "columns": [
	    	...
	    	// Diperbantukan
		var xin_table_commissions_help_ad = $('#xin_table_all_commissions_help_borongan').dataTable({
	        "bDestroy": true,
			"ajax": {
	            url : site_url+"payroll/salary_all_commissions_help_borongan/"+$('#user_id').val(),
	            type : 'GET'
	        },
	        "columns": [
				{"name": "kolom_1","orderable": false,"searchable": false,  "className": "text-center", "width": "8%"},
	    ...
	    // BPJS
		var xin_table_statutory_deductions_ad = $('#xin_table_all_statutory_deductions_borongan').dataTable({
	        "bDestroy": true,
			"ajax": {
	            url : site_url+"payroll/salary_all_statutory_deductions_borongan/"+$('#user_id').val(),
	            type : 'GET'
	        },
	        "columns": [
				{"name": "kolom_1","orderable": false,"searchable": false,  "className": "text-center", "width": "8%"},
	    ...
	    // Pemotongan
		var xin_table_minus_ad = $('#xin_table_all_minus_borongan').dataTable({
	        "bDestroy": true,
			"ajax": {
	            url : site_url+"payroll/salary_all_minus_borongan/"+$('#user_id').val(),
	            type : 'GET'
	        },
	         "columns": [
					{"name": "kolom_1","orderable": false,"searchable": false,  "className": "text-center", "width": "8%"},
	...
	// ======================================================================================================
		...
		jQuery("#employee_update_commissions_help").submit(function(e){
			...
			jQuery.ajax({
				type: "POST",
				url: e.target.action,
				data: obj.serialize()+"&is_ajax=4&data=employee_update_commissions_help&type=employee_update_commissions_help&form="+action,
				cache: false,
				success: function (JSON) {
					if (JSON.error != '') {
						...
						xin_table_commissions_help_ad.api().ajax.reload(function(){ 
							
							alert_success('Sukses',JSON.result);
							$('input[name="csrf_hris"]').val(JSON.csrf_hash);
						}, true);
						
						jQuery('#employee_update_commissions_help')[0].reset(); // To reset form fields
						jQuery('.save').prop('disabled', false);
					}
				}
			});
		...
		jQuery("#statutory_deductions_info_borongan").submit(function(e){
			...
			jQuery.ajax({
				type: "POST",
				url: e.target.action,
				data: obj.serialize()+"&is_ajax=4&data=statutory_deductions_info_borongan&type=statutory_deductions_info_borongan&form="+action,
				cache: false,
				success: function (JSON) {
					if (JSON.error != '') {
						...
						jQuery('#statutory_deductions_info_borongan')[0].reset(); // To reset form fields
						jQuery('.save').prop('disabled', false);
					}
				}
			});
	...
	// ======================================================================================================
		...
		$('.edit-modal-data').on('show.bs.modal', function (event) 
		{
			...
			if(field_tpe == 'salary_commissions_borongan'){
				var field_add = '&data=salary_commissions_info&type=salary_commissions_info&';

			} else if(field_tpe == 'salary_commissions_help_borongan'){
				var field_add = '&data=salary_commissions_help_info&type=salary_commissions_help_info&';

			} else if(field_tpe == 'salary_minus_borongan'){
				var field_add = '&data=salary_minus_info&type=salary_minus_info&';

			} else if(field_tpe == 'salary_statutory_deductions_borongan'){
				var field_add = '&data=salary_statutory_deductions_info_borongan&type=salary_statutory_deductions_info&';
	   ...
	   /* Delete data */
		$("#delete_record").submit(function(e){
			...
			if(tk_type == 'all_commissions_borongan'){
				var field_add = '&is_ajax=30&data=delete_record&type=delete_all_commissions_borongan&';
				var tb_name = 'xin_table_'+tk_type;

			} else if(tk_type == 'all_commissions_help_borongan'){
				var field_add = '&is_ajax=30&data=delete_record&type=delete_all_commissions_help_borongan&';
				var tb_name = 'xin_table_'+tk_type;

			} else if(tk_type == 'all_statutory_deductions_borongan'){
				var field_add = '&is_ajax=30&data=delete_record&type=delete_all_statutory_deductions_borongan&';
				var tb_name = 'xin_table_'+tk_type;

			} else if(tk_type == 'all_minus_borongan'){
				var field_add = '&is_ajax=30&data=delete_record&type=delete_all_minus_borongan&';
				var tb_name = 'xin_table_'+tk_type;	
